...
class PrunedWav2Vec2EncoderLayer(PrunedModel):
    ...
    def __init__(self, org_model, config, i_layer, **prune_params):
        ...
        self.attention = PrunedWav2Vec2Attention(org_model.attention, self.config, i_layer, **prune_params) 
        # import pdb;pdb.set_trace()
        self.layer_norm = org_model.layer_norm
        self.feed_forward = PrunedWav2Vec2FeedForward(org_model.feed_forward, self.config, i_layer, **prune_params)
...
class PrunedWav2Vec2Encoder(PrunedModel): 
    def __init__(self, org_model, config, **prune_params):
        ...
        self.layers = nn.ModuleList([PrunedWav2Vec2EncoderLayer(org_model.layers[i], self.config, i, **prune_params) for i in range(self.config.num_hidden_layers)])
        self.dropout = org_model.dropout
...
class PrunedWav2Vec2EncoderLayerStableLayerNorm(PrunedModel):
    def __init__(self, org_model, config, i_layer, **prune_params):
        ...
        # self.config = org_model.config
        self.config = config
        self.attention = PrunedWav2Vec2Attention(org_model.attention, config, i_layer, **prune_params)
        self.dropout = org_model.dropout
        self.layer_norm = org_model.layer_norm
        self.feed_forward = PrunedWav2Vec2FeedForward(org_model.feed_forward, config, i_layer, **prune_params)
        self.final_layer_norm = org_model.final_layer_norm
...
class PrunedWav2Vec2EncoderStableLayerNorm(PrunedModel):
    def __init__(self, org_model, config, **prune_params):
        ...
        # import pdb;pdb.set_trace()
        # self.gradient_checkpointing = False
        self.layers = nn.ModuleList([PrunedWav2Vec2EncoderLayerStableLayerNorm(org_model.layers[i], self.config, i,**prune_params) for i in range(self.config.num_hidden_layers)])
...
class PrunedWav2Vec2ForCTC(PrunedModel):
    ...
    def forward(
            ...
            for name, module in self.named_modules():
                if name.endswith('weight_pruneizer_saspg'): # or name.endswith('weight_pruneizer'): # or name.endswith('activation_pruner'):
                    ...
                    if not self.thre_model_path and self.fix_prob:
                        # Pruned str checkpoints (prune_ASR_w2v_mag) omit saspg pruner tensors; only copy when present.
        ...
        # MAG unstr: exact_size from mag_mask element counts (dense shape, pre-export).
        ...
        if self.current_steps % 500 == 0:
            _, _pr_msg = format_encoder_attn_ff_prune_ratio_message(
                ...
                extra=(
                    "MAG unstr dense-shape mag_mask count; "
                    f"ctc_loss={ctc_q_loss}, lmb*size_loss={self.lmb * size_loss}, "
                    f"total_loss={self.loss}"
                ),
            )
            print(_pr_msg)
