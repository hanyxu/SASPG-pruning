...
class PrunedWav2Vec2NoLayerNormConvLayer(PrunedModel):
    def __init__(self, org_model, layer_id=0, config=None, **prune_params):
        # self.prune_dict = prune_params['prune_dict']
...
class PrunedWav2Vec2LayerNormConvLayer(PrunedModel):
    def __init__(self, org_model, layer_id = 0, config=None, **prune_params):
        # self.prune_dict = prune_params['prune_dict']
...
class PrunedWav2Vec2GroupNormConvLayer(PrunedModel): # not used in Wav2vec2forCTC
    def __init__(self, org_model, layer_id=0, config=None, **prune_params):
        # self.prune_dict = prune_params['prune_dict']
...
class PrunedWav2Vec2FeatureEncoder(PrunedModel):
    ...
    def __init__(self, org_model, config, **prune_params):
        ...
        if  self.config.feat_extract_norm == "group":
            # self.conv_layers = PrunedWav2Vec2GroupNormConvLayer(org_model.conv_layers[:1], **prune_params) + PrunedWav2Vec2NoLayerNormConvLayer(org_model.conv_layers[1:], **prune_params)
            conv_layers = [PrunedWav2Vec2GroupNormConvLayer(org_model.conv_layers, layer_id = 0, config=self.config,**prune_params)] + [
                PrunedWav2Vec2NoLayerNormConvLayer(org_model.conv_layers, layer_id= i + 1, config=self.config, **prune_params) for i in range(config.num_feat_extract_layers - 1)
            ]
        elif self.config.feat_extract_norm == "layer":
            # self.conv_layers = PrunedWav2Vec2LayerNormConvLayer(org_model.conv_layers, **prune_params)
            conv_layers = [
                PrunedWav2Vec2LayerNormConvLayer(org_model.conv_layers, layer_id=i, config=self.config, **prune_params) for i in range(config.num_feat_extract_layers)
            ]   
        else:
            raise ValueError(
...
class PrunedWav2Vec2FeatureProjection(PrunedModel):
    def __init__(self, org_model, config, **prune_params):
        # self.prune_dict = prune_params['prune_dict']
        super().__init__()
...
class PrunedWav2Vec2PositionalConvEmbedding(PrunedModel):
    ...
    def __init__(self, org_model, **prune_params):
        ...
        self.padding = org_model.padding # Wav2Vec2SamePadLayer(org_model.num_conv_pos_embeddings)
...
class PrunedWav2Vec2FeedForward(PrunedModel):
    def __init__(self, org_model, config, **prune_params):
        ...
        self.weight_in = None
        self.is_load = prune_params['is_load']
        self.is_load_mask = False
        ...
        self.register_buffer('mask_1',torch.zeros((3072, 1)))
        self.register_buffer('mask_0_5',torch.zeros((3072, 1)))
        self.register_buffer('mask_0_25',torch.zeros((3072, 1)))
        self.register_buffer('mask_0_125',torch.zeros((3072, 1)))
        self.register_buffer('mask_0_75',torch.zeros((3072, 1)))
        self.register_buffer('mask_0_075',torch.zeros((3072, 1)))
        self.register_buffer('mask_0_1',torch.zeros((3072, 1)))
        
        self.value_0_5 = prune_params['value_0_5']
        self.value_0_75 = prune_params['value_0_75'] # warning !!!!!!!!!!!
        self.value_0_25 = prune_params['value_0_25']
        self.value_0_125 = prune_params['value_0_125']
        self.value_1 = prune_params['value_1']
        ...
        if not self.is_load:
            ...
            self.mask_1 = torch.ones((3072, 1), device='cuda')
            
            _, self.indices_0_75 = torch.topk(self.sum_channel_score, 2304)
            # self.mask_0_75 = torch.zeros((3072, 1), device='cuda')
            self.mask_0_75[self.indices_0_75] = 1
            
            
            _, self.indices_0_5 = torch.topk(self.sum_channel_score, 1536)
            # self.mask_0_5 = torch.zeros((3072, 1), device='cuda')
            self.mask_0_5[self.indices_0_5] = 1
            
            _, self.indices_0_25 = torch.topk(self.sum_channel_score, 768)
            # self.mask_0_25 = torch.zeros((3072, 1), device='cuda')
            self.mask_0_25[self.indices_0_25] = 1
            
            _, self.indices_0_125 = torch.topk(self.sum_channel_score, 384)
            # self.mask_0_125 = torch.zeros((3072, 1), device='cuda')
            self.mask_0_125[self.indices_0_125] = 1
            
            if self.value_0_075 != 0.:
                _, self.indices_0_1 = torch.topk(self.sum_channel_score, 307)
                # self.mask_0_1 = torch.zeros((3072, 1), device='cuda')
                self.mask_0_1[self.indices_0_1] = 1
                
                _, self.indices_0_075 = torch.topk(self.sum_channel_score, 230)
                # self.mask_0_075 = torch.zeros((3072, 1), device='cuda')
        ...
        self.intermediate_dense.weight_pruneizer_saspg.pruner.pruner.value_0_75 = self.value_0_75 
        self.output_dense.weight_pruneizer_saspg.pruner.pruner.value_0_75 = self.value_0_75 

        self.intermediate_dense.weight_pruneizer_saspg.pruner.pruner.value_0_075 = self.value_0_075 
        self.output_dense.weight_pruneizer_saspg.pruner.pruner.value_0_075 = self.value_0_075 
        
    def forward(self, hidden_states):
        # import pdb;pdb.set_trace()
        # print('self.eta_max,self.eta_min:', self.eta_max,self.eta_min)
        if self.training:
        ...
        if not self.is_load_mask:
            self.mask_1 = self.mask_1.cuda()
            self.mask_0_5 = self.mask_0_5.cuda()
            self.mask_0_75 = self.mask_0_75.cuda()
            self.mask_0_25 = self.mask_0_25.cuda()
            self.mask_0_125 = self.mask_0_125.cuda()
            if self.value_0_075 != 0:
                self.mask_0_1 = self.mask_0_1.cuda()
                self.mask_0_075 = self.mask_0_075.cuda()
            # self.prune_output = None
            
            self.intermediate_dense.weight_pruneizer_saspg.pruner.pruner.prune_output = torch.tensor(1)
            
            self.output_dense.weight_pruneizer_saspg.pruner.pruner.prune_output = torch.tensor(0)
            
            self.is_load_mask = True
    ...
    def prune(self):
        ...
        # import pdb;pdb.set_trace()
        assert self.sum_channel_score is not None
...
class PrunedWav2Vec2Attention(PrunedModel):
    ...
    def __init__(self, org_model, config, **prune_params):
        ...
        self.register_buffer('mask_1',torch.zeros((12, 1)))
        self.register_buffer('mask_0_5',torch.zeros((12, 1)))
        self.register_buffer('mask_0_25',torch.zeros((12, 1)))
        self.register_buffer('mask_0_125',torch.zeros((12, 1)))
        self.register_buffer('mask_0_75',torch.zeros((12, 1)))
        # self.register_buffer('mask_0_075',torch.zeros((12, 1)))
        # self.register_buffer('mask_0_1',torch.zeros((12, 1)))
        ...
        if not self.is_load:
            ...
            self.mask_1 = torch.ones((12, 1), device='cuda')
            # import pdb;pdb.set_trace()
            _, self.indices_0_75 = torch.topk(self.sum_channel_score.squeeze(-1), 9)
            # self.mask_0_75 = torch.zeros((12, 1), device='cuda')
            self.mask_0_75[self.indices_0_75] = 1
            
            _, self.indices_0_5 = torch.topk(self.sum_channel_score.squeeze(-1), 6)
            # self.mask_0_5 = torch.zeros((12, 1), device='cuda')
            self.mask_0_5[self.indices_0_5] = 1
            
            _, self.indices_0_25 = torch.topk(self.sum_channel_score.squeeze(-1), 3)
            # self.mask_0_25 = torch.zeros((12, 1), device='cuda')
            self.mask_0_25[self.indices_0_25] = 1
            
            _, self.indices_0_125 = torch.topk(self.sum_channel_score.squeeze(-1), 1)
            # self.mask_0_125 = torch.zeros((12, 1), device='cuda')
            self.mask_0_125[self.indices_0_125] = 1
        ...
        self.v_proj.weight_pruneizer_saspg.pruner.pruner.value_0_75 = self.value_0_75 
        self.out_proj.weight_pruneizer_saspg.pruner.pruner.value_0_75 = self.value_0_75
        
        self.k_proj.weight_pruneizer_saspg.pruner.pruner.value_0_075 = self.value_0_075 
        self.q_proj.weight_pruneizer_saspg.pruner.pruner.value_0_075 = self.value_0_075 
    ...
    def forward(
        ...
        if not self.is_load_mask:
            ...
            # self.q_proj.weight_pruneizer_saspg.pruner.pruner.mask_1 = self.mask_1.cuda()
            # self.q_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_5 = self.mask_0_5.cuda()
            # self.q_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_75 = self.mask_0_75.cuda()
            # self.q_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_25 = self.mask_0_25.cuda()
            # self.q_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_125 = self.mask_0_125.cuda()
            self.q_proj.weight_pruneizer_saspg.pruner.pruner.prune_output = torch.tensor(1)
            
            # self.k_proj.weight_pruneizer_saspg.pruner.pruner.mask_1 = self.mask_1.cuda()
            # self.k_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_5 = self.mask_0_5.cuda()
            # self.k_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_75 = self.mask_0_75.cuda()
            # self.k_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_25 = self.mask_0_25.cuda()
            # self.k_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_125 = self.mask_0_125.cuda()
            self.k_proj.weight_pruneizer_saspg.pruner.pruner.prune_output = torch.tensor(1)
            
            # self.v_proj.weight_pruneizer_saspg.pruner.pruner.mask_1 = self.mask_1.cuda()
            # self.v_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_5 = self.mask_0_5.cuda()
            # self.v_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_75 = self.mask_0_75.cuda()
            # self.v_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_25 = self.mask_0_25.cuda()
            # self.v_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_125 = self.mask_0_125.cuda()
            self.v_proj.weight_pruneizer_saspg.pruner.pruner.prune_output = torch.tensor(1)
            
            # self.out_proj.weight_pruneizer_saspg.pruner.pruner.mask_1 = self.mask_1.cuda()
            # self.out_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_5 = self.mask_0_5.cuda()
            # self.out_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_75 = self.mask_0_75.cuda()
            # self.out_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_25 = self.mask_0_25.cuda()
            # self.out_proj.weight_pruneizer_saspg.pruner.pruner.mask_0_125 = self.mask_0_125.cuda()
            self.out_proj.weight_pruneizer_saspg.pruner.pruner.prune_output = torch.tensor(0)
            self.is_load_mask = True
        # count = torch.sum(self.mask_0_5 == 1).item()
        # print("Number of self.mask_0_5 in the tensor:", count)
        # count = torch.sum(self.mask_0_25 == 1).item()
        # print("Number of self.mask_0_25 in the tensor:", count)
        # count = torch.sum(self.mask_0_125 == 1).item()
        # print("Number of self.mask_0_125 in the tensor:", count)
        
        # if key_value_states are provided this layer is used as a cross-attention layer
        # for the decoder
        is_cross_attention = key_value_states is not None
        bsz, tgt_len, _ = hidden_states.size() # hidden_states shape [1, 911, 768] x 768, 768 # the row proj of is divided by 12
        # import pdb;pdb.set_trace()
        # get query proj
        ...
        # before: q: [1, 911, 768] k:torch.Size([1, 12, 911, 64]) # torch.Size([1, 12, 911, 64])
        
        proj_shape = (bsz * self.num_heads, -1, self.head_dim) # (12, -1, 64)
        query_states = self._shape(query_states, tgt_len, bsz).view(*proj_shape) # -> torch.Size([12, 911, 64])
        key_states = key_states.view(*proj_shape) # -> torch.Size([12, 911, 64])
        value_states = value_states.view(*proj_shape) # -> torch.Size([12, 911, 64])

        src_len = key_states.size(1) # 911
        attn_weights = torch.bmm(query_states, key_states.transpose(1, 2)) # 12, 911, 911
        ...
        # attention_weights = self.attn_probs_act_pruneizer(attention_weights)
        ...
        # attn_probs = self.attn_probs_act_pruneizer(attn_probs)

        # attn_probs = nn.Dropout(p=self.dropout)(attn_weights)

        attn_output = torch.bmm(attn_probs, value_states) # 12, 911, 64
        ...
        attn_output = attn_output.view(bsz, self.num_heads, tgt_len, self.head_dim) # 1, 12, 911, 64
        ...
        # (12,1) -> (12,1,1)
        # if self.training:
        #         attn_output = attn_output * self.mask_channel.unsqueeze(-1).unsqueeze(-1)
        #     else:
        #         attn_output = attn_output * self.compiled_mask.unsqueeze(-1).unsqueeze(-1)


        attn_output = attn_output.transpose(1, 2) # 1,911,12,64

        # Use the `embed_dim` from the config (stored in the class) rather than `hidden_state` because `attn_output` can be
        # partitioned aross GPUs when using tensor-parallelism.
        attn_output = attn_output.reshape(bsz, tgt_len, self.embed_dim) # 1, 911, 12x64
        # print('get out proj')
        attn_output = self.out_proj(attn_output) # out should be transed
    ...
    def prune(self):
        ...
        if self.sum_channel_score is not None:
            ...
            head_mask = (self.mask_channel == 1.0)
...
class PrunedWav2Vec2EncoderLayer(PrunedModel):
    ...
    def __init__(self, org_model, config, **prune_params):
        ...
        self.attention = PrunedWav2Vec2Attention(org_model.attention, self.config, **prune_params) 
        # import pdb;pdb.set_trace()
        self.layer_norm = org_model.layer_norm
        self.feed_forward = PrunedWav2Vec2FeedForward(org_model.feed_forward, self.config, **prune_params)
...
class PrunedWav2Vec2Encoder(PrunedModel): 
    def __init__(self, org_model, config, **prune_params):
        super().__init__()
        ...
        self.pos_conv_embed = PrunedWav2Vec2PositionalConvEmbedding(org_model.pos_conv_embed, **prune_params)
        ...
        self.layers = nn.ModuleList([PrunedWav2Vec2EncoderLayer(org_model.layers[i], self.config, **prune_params) for i in range(self.config.num_hidden_layers)])
        self.dropout = org_model.dropout
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            skip_the_layer = True if self.is_skip and self.training and (dropout_probability < self.config.layerdrop) else False
            # print('num_layer',self.num_layer % 12, skip_the_layer)
            # self.num_layer += 1
        ...
        # if self.is_skip == False and self.training:
        #     self.is_skip = True
...
class PrunedWav2Vec2AdapterLayer(PrunedModel):
    def __init__(self, org_model, **prune_params):
        super().__init__()
...
class PrunedWav2Vec2Adapter(PrunedModel):
    def __init__(self, org_model, **prune_params):
        super().__init__()
        ...
        self.layers = nn.ModuleList(PrunedWav2Vec2AdapterLayer(org_model[i].layers, ) for i in range(self.config.num_adapter_layers))
...
class PrunedWav2Vec2EncoderLayerStableLayerNorm(PrunedModel):
    def __init__(self, org_model, config, **prune_params):
        super().__init__()
        ...
        # self.config = org_model.config
        self.config = config
        self.attention = PrunedWav2Vec2Attention(org_model.attention, config, **prune_params)
        self.dropout = org_model.dropout
        self.layer_norm = org_model.layer_norm
        self.feed_forward = PrunedWav2Vec2FeedForward(org_model.feed_forward, config, **prune_params)
        self.final_layer_norm = org_model.final_layer_norm
...
class PrunedWav2Vec2EncoderStableLayerNorm(PrunedModel):
    def __init__(self, org_model, config, **prune_params):
        super().__init__()
        ...
        # self.layers = nn.ModuleList([PrunedWav2Vec2EncoderLayerStableLayerNorm(org_model[i].layers, **prune_params) for i in range(self.config.num_hidden_layers)])
        # self.layers = prune_model(org_model.layers, specials={Wav2Vec2EncoderLayerStableLayerNorm: PrunedWav2Vec2EncoderLayerStableLayerNorm}, **prune_params)
        # import pdb;pdb.set_trace()
        # self.gradient_checkpointing = False
        self.layers = nn.ModuleList([PrunedWav2Vec2EncoderLayerStableLayerNorm(org_model.layers[i], self.config, **prune_params) for i in range(self.config.num_hidden_layers)])
...
class PrunedWav2Vec2Model(PrunedModel):
    def __init__(self, org_model,  **prune_params):
        super().__init__()
        # self.config = config
        # import pdb;pdb.set_trace()
        ...
        # import pdb;pdb.set_trace()
        self.feature_extractor = PrunedWav2Vec2FeatureEncoder(org_model.feature_extractor, self.config, **prune_params)
        self.feature_projection = PrunedWav2Vec2FeatureProjection(org_model.feature_projection, self.config, **prune_params)
        ...
        if self.config.do_stable_layer_norm:
            # raise NotImplementedError('current branch of computation is not yet supported')
            # not used
            self.encoder = PrunedWav2Vec2EncoderStableLayerNorm(org_model.encoder, self.config, **prune_params)
        else:
            self.encoder = PrunedWav2Vec2Encoder(org_model.encoder, self.config, **prune_params)

        # import pdb;pdb.set_trace()
        # self.adapter = PrunedWav2Vec2Adapter(org_model.adapter, **prune_params) if config.add_adapter else None
        # import pdb;pdb.set_trace()

        if self.config.add_adapter is True:
            self.adapter = PrunedWav2Vec2Adapter(org_model.adapter, self.config, **prune_params)
        else:
            self.adapter = None
    ...
    def forward(
        ...
        if not return_dict:
            # return hidden_states, self.res1_act_pruneizer((hidden_states, extract_features) + encoder_outputs[1:])
...
class PrunedWav2Vec2ForCTC(PrunedModel):
    def __init__(self, org_model, prune_setup=None, precision_levels=[8,4], **prune_params):
        ...
        # prune_params.pop('channel_pruning10')
        
        # prune_params.pop('prob_1')
        # prune_params.pop('prob_05')
        # prune_params.pop('prob_025')
        # prune_params.pop('prob_0125')
        ...
        self.sum_shape =  768*768*4*12 + 768*3072*2*12 + 3072*12 + 768*3*12 # weight + bias
        ...
        if 'model_path' in prune_params:
            ...
            # print(self.weight_dict['wav2vec2.encoder.layers.0.attention.q_proj.weight'])
            # self.delete_dict()
            # self.clean_dict()
            self.model_path = True
            prune_params['is_load'] = True
        ...
        self.wav2vec2 = PrunedWav2Vec2Model(org_model.wav2vec2, **prune_params)
        # import pdb; pdb.set_trace()
        ...
        # import pdb;pdb.set_trace()
        # if 'q_head' in self.prune_dict and prune_setup != 'FP_logits':
        #     self.lm_head = prune_model(org_model.lm_head, **prune_params_)
        #     # import pdb;pdb.set_trace()            
        # elif 'baseline' in self.prune_dict or any(key in self.prune_dict for key in ['only_ext', 'only_t', 'only_proj']):
        #     self.lm_head = org_model.lm_head
            
        # elif prune_setup in ['MSE_logits', 'FP_logits', 'all']:
        #     if prune_setup == 'MSE_logits':
        #         prune_params_['act_range_method'] = RangeEstimators.MSE
        #         prune_params_['act_range_options'] = dict(opt_method=OptMethod.golden_section)
        #     elif prune_setup == 'FP_logits':
        #         print('Do not prune output of FC layer')
        #         self.lm_head.activation_pruner = FP32Acts()
        #     elif prune_setup == 'all' and not any(key in self.prune_dict for key in ['only_ext', 'only_t', 'only_proj']):
        #         pass
        #     self.lm_head = prune_model(org_model.lm_head, **prune_params_)
            
        # else:
        #     raise ValueError("Pruning setup '{}' not supported.".format(prune_setup))
    ...
    def forward(
        ...
        """if not self.training and self.calib < self.num_acts: # forward 2 bit to get the initial value 
            ...
            loss = []
            logits =[]
            # self.wav2vec2.set_precision_level_mask(23)
            ...
            for precision in self.precision_levels:
                ...
                self.wav2vec2.set_precision_level_direct(precision)
                # self.lm_head.set_precision_level_direct(precision)
                
                # self.wav2vec2.set_precision_level_mask(30)
                # self.lm_head.set_precision_level_mask(30)
                
                # import pdb;pdb.set_trace()
                self.wav2vec2.return_q()
        ...
        if self.model_path:
            ...
            # PrunedWav2Vec2Attention().is_load = True
            # PrunedWav2Vec2Feedforward().is_load = True
            # prune_params['is_load'] = True
            ...
            for name, module in self.named_modules():
                if name.endswith('weight_pruneizer'): # or name.endswith('activation_pruner'):
                    ...
                    # module.pruner.pruner.x_prune_mask.data = self.weight_dict[f'{name}.pruner.pruner.x_prune_mask']

                    if index_delta == 1:
                        module.pruner.pruner._delta.data = self.weight_dict[f'{name}.pruner.pruner._delta']
                        module.pruner.pruner._delta_2bit.data = self.weight_dict[f'{name}.pruner.pruner._delta_2bit']
                        module.pruner.pruner._delta_4bit.data = self.weight_dict[f'{name}.pruner.pruner._delta_4bit']
                        module.pruner.pruner._signed.data = self.weight_dict[f'{name}.pruner.pruner._signed']
                    
                    if not self.thre_model_path and self.fix_prob:
                        # import pdb;pdb.set_trace()
                        module.pruner.pruner.threshold_2.data = self.weight_dict[f'{name}.pruner.pruner.threshold_2']
                        module.pruner.pruner.threshold_4.data = self.weight_dict[f'{name}.pruner.pruner.threshold_4']
                        module.pruner.pruner.threshold_8.data = self.weight_dict[f'{name}.pruner.pruner.threshold_8']
        ...
        if self.thre_model_path:
            ...
            # self.PrunedWav2Vec2Attention.is_load = True
            # self.PrunedWav2Vec2Feedforward.is_load = True
            # prune_params['is_load'] = True
            ...
            for name, module in self.named_modules():
                if name.endswith('weight_pruneizer_saspg'): # or name.endswith('activation_pruner'):
                    ...
                    module.pruner.pruner.current_ratio.data = self.weight_dict[f'{name}.pruner.pruner.current_ratio']
                    print(name, self.weight_dict[f'{name}.pruner.pruner.current_ratio'],  module.pruner.pruner.current_ratio)
        ...
        size_loss = self.wav2vec2.get_gate_loss_prune_channel()
        ...
        # cos_loss = self.wav2vec2.get_gate_loss_cos()
        
        """if not self.fix_prob:
            mixed_precision_list = [i for i in self.wav2vec2.get_mixed_sparsity()]
        else:
            mixed_precision_list = [i for i in self.wav2vec2.get_mixed_sparsity_fix()]"""
        ...
        mixed_thre_list = [i for i in self.wav2vec2.get_mixed_thre_fix()]
    ...
    def subforward(
        ...
        # self.wav2vec2.set_precision_level_direct(precision)
        #     # self.wav2vec2.set_precision_level(precision)
        # self.lm_head.set_precision_level_direct(precision)
