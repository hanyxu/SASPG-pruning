...
class PrunedWav2Vec2FeedForward(PrunedModel):
    def __init__(self, org_model, config, **prune_params):
        ...
        """if self.mask is None and not self.load:
            if self.weight_out is None:
                # import pdb;pdb.set_trace()
                self.weight_out = torch.square(self.output_dense.weight).sum(dim=0)
                self.weight_out = self.weight_in + self.weight_out

                _, self.indices = torch.topk(self.weight_out, 2048)
                self.mask = torch.zeros((3072, 1), device='cuda')
                with open('/project_bdda8/bdda/hnxu/PruneWav2vec2/transformer-pruning/pruning/weight_indice_L2_final_2048.txt', 'a') as file:
                    file.write(' '.join(map(str, self.indices.tolist())) + '\n')

                    # # 读取a.txt文件的最后一行到indices_1变量
                self.mask[self.indices] = 1
                # print('indices',len(self.indices))
                # print(self.mask)
                # import pdb;pdb.set_trace()
                self.output_dense.mask = self.mask
                self.intermediate_dense.mask = self.mask"""
...
class PrunedWav2Vec2Attention(PrunedModel):
    ...
    def forward(
        ...
        # pruner
        # attn_weights = self.attn_scores_act_pruneizer(attn_weights)
        # MACs: 14400/768/12
        ...
        # attn_weights = self.attn_probs_act_pruneizer(attn_weights)
        ...
        # attn_output = self.context_act_pruneizer(attn_output) 
        # print('get out proj')
...
class PrunedWav2Vec2EncoderLayer(PrunedModel):
    ...
    def forward(self, hidden_states, attention_mask=None, output_attentions=False):
        ...
        # if self.res_selfout_act_pruneizer:
        #     hidden_states = self.res_selfout_act_pruneizer(hidden_states)

        hidden_states = self.layer_norm(hidden_states)
        hidden_states = hidden_states + self.feed_forward(hidden_states)
        # if self.res_ff_act_pruneizer:
        #     hidden_states = self.res_ff_act_pruneizer(hidden_states)
...
class PrunedWav2Vec2Encoder(PrunedModel): 
    def __init__(self, org_model, config, **prune_params):
        ...
        self.layer_norm = org_model.layer_norm
        self.sum_pos_embd_act_pruneizer = None
    ...
    def forward(
        ...
        # if self.sum_pos_embd_act_pruneizer:
        #     hidden_states = self.sum_pos_embd_act_pruneizer(hidden_states)
        hidden_states = self.layer_norm(hidden_states)
        hidden_states = self.dropout(hidden_states)
...
class PrunedWav2Vec2ForCTC(PrunedModel):
    def __init__(self, org_model, prune_setup=None, precision_levels=[8,4], **prune_params):
        ...
        self.sum_shape =  768*768*4*12 + 768*3072*2*12
        ...
        self.avg_ce_loss = 0
        self.avg_c_loss = 0
        self.n_bits = prune_params['n_bits']
    ...
    def forward(
        ...
        if self.model_path:
            ...
            for name, module in self.named_modules():
                if name.endswith('weight_pruneizer_saspg'): # or name.endswith('weight_pruneizer'): # or name.endswith('activation_pruner'):
                    # print(name)  # 打印模块的名称
                    ...
                    # module.pruner.pruner.threshold_prune.data = self.weight_dict[f'{name}.pruner.pruner.threshold_prune']
                    # print(module.pruner.pruner.threshold_prune.data,self.weight_dict[f'{name}.pruner.pruner.threshold_prune'] )
