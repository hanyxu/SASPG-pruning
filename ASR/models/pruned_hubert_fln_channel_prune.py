...
class PrunedHubertFeedForward(PrunedModel):
    def __init__(self, org_model, config, **prune_params):
        ...
        self.register_buffer('mask_1',torch.zeros((4096, 1)))
        self.register_buffer('mask_0_5',torch.zeros((4096, 1)))
        self.register_buffer('mask_0_25',torch.zeros((4096, 1)))
        self.register_buffer('mask_0_125',torch.zeros((4096, 1)))
        self.register_buffer('mask_0_75',torch.zeros((4096, 1)))
        self.register_buffer('mask_0_075',torch.zeros((4096, 1)))
        self.register_buffer('mask_0_1',torch.zeros((4096, 1)))
    ...
    def prune(self):
        # Physical FFN channel prune (was mistakenly placed under Attention).
        new_config = {
...
class PrunedHubertAttention(PrunedModel):
    ...
    def __init__(self, org_model, config, **prune_params):
        ...
        self.register_buffer('mask_1',torch.zeros((16, 1)))
        self.register_buffer('mask_0_5',torch.zeros((16, 1)))
        self.register_buffer('mask_0_25',torch.zeros((16, 1)))
        self.register_buffer('mask_0_125',torch.zeros((16, 1)))
        self.register_buffer('mask_0_75',torch.zeros((16, 1)))
        self.register_buffer('mask_0_075',torch.zeros((16, 1)))
        self.register_buffer('mask_0_1',torch.zeros((16, 1)))
    ...
    def prune(self):
        # Physical attention head prune (was mistakenly placed under FeedForward).
        new_config = {
            "use_attention": True,
...
class PrunedHubertEncoderLayer(PrunedModel):
    ...
    def __init__(self, org_model, config, **prune_params):
        ...
        self.attention = PrunedHubertAttention(org_model.attention, self.config, **prune_params) 
        
        self.layer_norm = org_model.layer_norm
        self.feed_forward = PrunedHubertFeedForward(org_model.feed_forward, self.config, **prune_params)
...
class PrunedHubertEncoder(PrunedModel): 
    def __init__(self, org_model, config, **prune_params):
        ...
        self.layers = nn.ModuleList([PrunedHubertEncoderLayer(org_model.layers[i], self.config, **prune_params) for i in range(self.config.num_hidden_layers)])
        self.dropout = org_model.dropout
...
class PrunedHubertEncoderLayerStableLayerNorm(PrunedModel):
    def __init__(self, org_model, config, **prune_params):
        ...
        # self.config = org_model.config
        self.config = config
        self.attention = PrunedHubertAttention(org_model.attention, config, **prune_params)
        self.dropout = org_model.dropout
        self.layer_norm = org_model.layer_norm
        self.feed_forward = PrunedHubertFeedForward(org_model.feed_forward, config, **prune_params)
        self.final_layer_norm = org_model.final_layer_norm
...
class PrunedHubertEncoderStableLayerNorm(PrunedModel):
    def __init__(self, org_model, config, **prune_params):
        ...
        # import pdb;pdb.set_trace()
        self.gradient_checkpointing = False
        self.layers = nn.ModuleList([PrunedHubertEncoderLayerStableLayerNorm(org_model.layers[i], self.config, **prune_params) for i in range(self.config.num_hidden_layers)])
...
class PrunedHubertForCTC(PrunedHubertPreTrainedModel):
    def __init__(self, org_model, prune_setup=None, precision_levels=[8,4], **prune_params):
        ...
        self.sum_shape =  1024*1024*4*24 + 1024*4096*2*24 + 4096*24 + 1024*3*24
        self.first_round = True
