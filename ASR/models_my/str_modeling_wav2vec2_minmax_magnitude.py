...
class Wav2Vec2ForPreTrainingOutput(ModelOutput):
    ...
    Args:
        ...
        projected_states (`torch.FloatTensor` of shape `(batch_size, sequence_length, config.proj_codevector_dim)`):
            ...
            projected pruned states.
        projected_pruned_states (`torch.FloatTensor` of shape `(batch_size, sequence_length, config.proj_codevector_dim)`):
            Pruned extracted feature vectors projected to *config.proj_codevector_dim* representing the positive
    ...
    projected_pruned_states: torch.FloatTensor = None
...
class Wav2Vec2FeatureEncoder(nn.Module):
    ...
    # 7 layers CNN feature encoder
    # config.num_feat_extract_layers = 7
    def __init__(self, config):
        super().__init__()
...
class Wav2Vec2Attention(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer('compiled_mask', torch.ones((12, 1), dtype=torch.float32))
    ...
    def get_num_params_unstr(self): # NOTE:see if this is ok here
        ...
        # print('1 self.mask_channel:',torch.sum(self.mask_channel))
...
class Wav2Vec2FeedForward(nn.Module):
    ...
    # self.output_dense = nn.Linear(config.intermediate_size, config.hidden_size)
        ...
        self.register_buffer('prune_ratio', torch.tensor(1.))
        self.register_buffer('compiled_mask', torch.ones((3072,), dtype=torch.float32))
        self.register_buffer('mag_pruned', torch.tensor(0.))
        ...
        # self.mask_channel = torch.ones(3072, device="cuda" if self.cuda else "cpu")
...
class Wav2Vec2EncoderLayer(nn.Module):
    def __init__(self, config, i_layer):
        super().__init__()
        # if config.pruned_attention_heads == [12]*12:
        if config.prune:
        ...
        # if config.pruned_ffn_inter == [3072]*12:
        if config.prune:  
            self.feed_forward = Wav2Vec2FeedForward(config, prune_intermediate=True, i_layer=i_layer)
        else:
            self.feed_forward = Wav2Vec2FeedForward(config, prune_intermediate=False, i_layer=i_layer)
...
class Wav2Vec2Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout)
        self.layers = nn.ModuleList([Wav2Vec2EncoderLayer(config, i_layer=i) for i in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False
...
class Wav2Vec2EncoderStableLayerNorm(nn.Module):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [Wav2Vec2EncoderLayerStableLayerNorm(config, i_layer=i) for i in range(config.num_hidden_layers)]
        )
        self.gradient_checkpointing = False
...
class Wav2Vec2GumbelVectorPruner(nn.Module):
    """
    Vector pruning using gumbel softmax. See `[CATEGORICAL REPARAMETERIZATION WITH
...
class Wav2Vec2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, Wav2Vec2GumbelVectorPruner):
...
@add_start_docstrings("""Wav2Vec2 Model with a pruner and `VQ` head on top.""", WAV_2_VEC_2_START_DOCSTRING)
class Wav2Vec2ForPreTraining(Wav2Vec2PreTrainedModel):
    def __init__(self, config: Wav2Vec2Config):
        super().__init__(config)
        self.wav2vec2 = Wav2Vec2Model(config)
        self.dropout_features = nn.Dropout(config.feat_pruner_dropout)

        self.pruner = Wav2Vec2GumbelVectorPruner(config)
    ...
    def set_gumbel_temperature(self, temperature: int):
        ...
        """
        self.pruner.temperature = temperature
    ...
    def forward(
        ...
        sampled_negative_indices (`torch.BoolTensor` of shape `(batch_size, sequence_length, num_negatives)`, *optional*):
            Indices indicating which pruned target vectors are used as negative sampled vectors in contrastive loss.
        ...
        >>> # compute cosine similarity between predicted (=projected_states) and target (=projected_pruned_states)
        >>> cosine_sim = torch.cosine_similarity(outputs.projected_states, outputs.projected_pruned_states, dim=-1)
        ...
        # 2. prune all (unmasked) extracted features and project to final vq dim
        ...
        pruned_features, codevector_perplexity = self.pruner(
            extract_features, mask_time_indices=mask_time_indices
        )
        pruned_features = self.project_q(pruned_features)

        loss = contrastive_loss = diversity_loss = None
        if sampled_negative_indices is not None:
            batch_size, sequence_length, hidden_size = pruned_features.shape

            # for training, we sample negatives
            # 3. sample K negatives (distractors) pruned states for contrastive loss
            # if attention_mask is passed, make sure that padded feature vectors cannot be sampled
            # sample negative pruned vectors BTC => (BxT)C
            negative_pruned_features = pruned_features.view(-1, hidden_size)[
                sampled_negative_indices.long().view(-1)
            ]
            negative_pruned_features = negative_pruned_features.view(
            ...
            neg_is_pos = (pruned_features == negative_pruned_features).all(-1)
        ...
        if not return_dict:
            if loss is not None:
                return (loss, transformer_features, pruned_features, codevector_perplexity) + outputs[2:]
            return (transformer_features, pruned_features, codevector_perplexity) + outputs[2:]

        return Wav2Vec2ForPreTrainingOutput(
            loss=loss,
            projected_states=transformer_features,
            projected_pruned_states=pruned_features,
...
class Wav2Vec2ForCTC(Wav2Vec2PreTrainedModel):
    def __init__(self, config, target_lang: Optional[str] = None):
        ...
        # if config.pruned_attention_heads == [12]*12 and config.pruned_ffn_inter == [3072]*12 :
        if config.prune:
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.already_pruned:
                ...
                # loss_reg = (self.lambda1 * (current_sparsity - cur_target_sparsity) + self.lambda2 * (current_sparsity - cur_target_sparsity)**2)

                # loss = 0.1 * ctc_loss + loss_reg
                # print(' cur_target_sparsity:', cur_target_sparsity)
        ...
        #     # import pdb;pdb.set_trace()
        #     if self.already_pruned:
        #         print(f'str_export post-pruned fine-tuning -- cur_sparsity: {current_sparsity}, ctc loss:{ctc_loss}, sum loss:{loss}, current_parms:{current_params}, total_params:{self.total_params}, lmbda1:{self.lambda1}, lmbda2:{self.lambda2}')
        #     else:
        #         print(f'str_export pruning -- cur_target_sparsity: {cur_target_sparsity}, cur_sparsity: {current_sparsity}, ctc loss:{ctc_loss}, sum loss:{loss}, current_parms:{current_params}, total_params:{self.total_params}, lmbda1:{self.lambda1},lmbda2:{self.lambda2}')
        # print('self.current_steps+=1')
