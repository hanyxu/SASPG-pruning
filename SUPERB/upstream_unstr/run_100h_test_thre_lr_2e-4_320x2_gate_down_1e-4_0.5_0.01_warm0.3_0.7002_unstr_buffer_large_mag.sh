...
teacher_ckpt=${DPHuBERT_PRETRAINED_ROOT}/hubert-large-ll60k.hf.pth    # checkpoint path
...
gate_lr=1e-4             
target_sparsity=0.7002    # final target sparsity
...
root_dir=exp_minmax_hubert_large_unstr_mag/hubert-large_${train_subset}_sp${target_sparsity}_spup${sparsity_warmup}_lr${lr}_up${warmup}_max${max}_${distill_mode}${distill_layers}_reglr${reg_lr}_gatelr${gate_lr}_${pruning_units}_sec${seconds_per_batch}_accum${accum_grad}_${eta_max}_${eta_min}_mag_finalwrm${final_warmup}_finalmax${final_max}_saveratio${save_ratio}_${final_lr}
...
python prune_1_01_large_mag.py \
    --eta_min ${eta_min} \
    --eta_max ${eta_max} \
...
cp -r "${DPHuBERT_PRETRAINED_ROOT}/pruned_hubert_large.pth" ${root_dir}/ckpts/pruned_hubert_large.pth
