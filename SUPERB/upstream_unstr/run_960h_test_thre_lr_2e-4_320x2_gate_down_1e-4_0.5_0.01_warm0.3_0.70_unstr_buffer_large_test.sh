...
sparsity_warmup=1    # warmup steps for sparsity; sparsity will linearly increase from 0 to target
seconds_per_batch=60
accum_grad=5
eta_max=0.5
eta_min=0.01

root_dir=exp_minmax_unstr_new_hubert_large/hubert-large_${train_subset}_sp${target_sparsity}_spup${sparsity_warmup}_lr${lr}_up${warmup}_max${max}_${distill_mode}${distill_layers}_reglr${reg_lr}_gatelr${gate_lr}_${pruning_units}_sec${seconds_per_batch}_accum${accum_grad}_gate_norm_${eta_max}_${eta_min}_tau_gate_up_30%_down_re_unstr_register_buffer_real_test

# 之前是10 tau 0.5-> 0.01 50000tausteps
# final distill config
final_lr=0.0001         # learning rate for final distillation (training step 2)
final_warmup=1       # warmup steps
final_max=2         # max update steps
