...
gate_lr=1e-4             
target_sparsity=0.7503    # final target sparsity
...
if [ "$train_subset" = "train960" ]; then
    # save_ratio=0.2688
    # save_ratio=0.2660 # 93777656.
    # save_ratio=0.2657 # 93687088.
    # save_ratio=0.2656 # 93656888.
    # save_ratio=0.26565 # 93671920.
    save_ratio=0.268657 # 32255068.
    save_ratio=0.2430 # 30075912.、
    save_ratio=0.16664 # 23590300.
    # save_ratio=0.16653 
elif [ "$train_subset" = "train100" ]; then
    save_ratio=0.16653 # 23580964.
else
...
root_dir=exp_minmax_wavlm_base_unstr_mag/wavlm-base_${train_subset}_sp${target_sparsity}_spup${sparsity_warmup}_lr${lr}_up${warmup}_max${max}_${distill_mode}${distill_layers}_reglr${reg_lr}_gatelr${gate_lr}_${pruning_units}_sec${seconds_per_batch}_accum${accum_grad}_${eta_max}_${eta_min}_mag_finalwrm${final_warmup}_finalmax${final_max}_saveratio${save_ratio}_${final_lr}
final_exp_dir=${root_dir}/lr${final_lr}_up${final_warmup}_max${final_max}_unstr
...
python final_distill_1_01_mag.py \
    --tsv_dir ${tsv_dir} \
    --train_subset ${train_subset} \
    --seconds_per_batch ${seconds_per_batch} \
    --num_workers 12 \
    ...
    --l1_weight ${l1_weight} \
    --cos_weight ${cos_weight} \
    --cos_type ${cos_type} 2>&1 | tee ${final_exp_dir}/final_distill_prune_true_1_01_1.log || exit 1;
