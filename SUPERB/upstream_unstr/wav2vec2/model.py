...
class Wav2Vec2Model(Module):
    ...
    def detect_padding(self, tensor):
    
        assert tensor.dim() == 2, "输入必须是二维张量"
        ...
        for i in range(batch_size):
            # 获取当前样本的非零位置
            nz_indices = torch.where(non_zero_mask[i])[0]
            ...
            # 获取有效数据范围
            first_nz = nz_indices[0].item()
            last_nz = nz_indices[-1].item()
            
            # 情况2: 检查中间是否有零
            expected_indices = torch.arange(first_nz, last_nz+1)
            if not torch.equal(nz_indices, expected_indices):
                results.append("invalid")
                continue
                
            # 确定填充类型
            front_pad = first_nz > 0
            back_pad = last_nz < seq_len - 1
            
            if not front_pad and not back_pad:
                results.append("no_padding")
            elif front_pad and not back_pad:
                results.append("front_padding") 
            elif not front_pad and back_pad:
                results.append("back_padding")
            else:
                results.append("both_padding")
                
        return results
    def check_padding_consistency(self, tensor: torch.Tensor):
       
        assert tensor.dim() == 2, "Input must be a 2D tensor [batch, seq_len]"
        batch_size, seq_len = tensor.shape
        
        # Track padding patterns for all samples
        padding_modes = []
        ...
        for idx in range(batch_size):
            ...
            nz_indices = torch.nonzero(sample, as_tuple=True)[0]
            if len(nz_indices) == 0:
                raise AssertionError(f"Sample {idx} contains all zeros (invalid padding)")
                
            # Get first/last non-zero positions
            first_nz = nz_indices[0].item()
            last_nz = nz_indices[-1].item()
        ...
        unique_modes = set(padding_modes)
        if len(unique_modes) != 1:
            raise AssertionError(f"Mixed padding patterns detected: {unique_modes}")
    ...
    def extract_features(
        ...
        # print('self.normalize_waveform:',self.normalize_waveform)
        # print(type(waveforms), waveforms, len(waveforms), waveforms.shape)
        # print(self.check_padding_consistency(waveforms)) 
