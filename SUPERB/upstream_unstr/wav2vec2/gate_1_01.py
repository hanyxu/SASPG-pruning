        ...
        if self.training:
            ...
            if not self.reset_threshold:
                if not self.init_sparsity:
                    ...
                    sorted_scores, _ = torch.sort(self.weight)
            ...
            if self.weight is None:
                self.mask = torch.ones(3072, device=self.gate_threshold.device)
            else:
                self.mask = torch.sigmoid((self.weight**2 - self.gate_threshold**2)/self.tau)
