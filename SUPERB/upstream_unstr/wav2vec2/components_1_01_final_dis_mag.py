...
def compute_unstructured_mask(weight, keep_ratio):
    """
    计算非结构化mask：保留绝对值最大的 keep_ratio 比例的权重。
    """
    ...
    # 确定要保留的数量 k
    k = int(round(num_params * keep_ratio))
    ...
    # 找到第 k 大的值作为阈值
    # 注意：如果存在多个相同值的权重刚好跨越阈值，topk可以确保选出k个
    topk_values, _ = torch.topk(flat_weight, k)
    threshold = topk_values[-1]
    
    # 生成 mask (1 for keep, 0 for prune)
    mask = (weight.abs() >= threshold).float()
    
    return mask

def print_zero_ratio_and_verify(tensor, name):
    """Print the ratio of zeros in a tensor, verify total ratio, and count pruned parameters"""
    total_elements = tensor.numel()
    zeros_count = torch.sum(tensor == 0).item()
    ones_count = torch.sum(tensor == 1).item()
    
    zero_ratio = zeros_count / total_elements
    one_ratio = ones_count / total_elements
    
    # Verify if the sum of zero and one ratios is approximately 1
    sum_ratio = zero_ratio + one_ratio
    is_valid = abs(sum_ratio - 1.0) < 1e-6
    
    # Print zero ratio in percentage format with 2 decimal places
    # print(f'{name} 0/all: {zero_ratio:.2%}')
    
    # Calculate pruned parameters (zeros) in millions
    pruned_params_millions = zeros_count / 1e6
    
    # Print pruned parameters with 2 decimal places
    # print(f'{name} Pruned Parameters: {pruned_params_millions:.2f}M')
    
    # If the sum is not 1, print a warning message
    if not is_valid:
        print(f"WARNING: Sum of zero and one ratios for {name} is not 1 ({sum_ratio:.6f})")
        print(f"      Zero ratio: {zero_ratio:.6f}, One ratio: {one_ratio:.6f}, Other values: {1-sum_ratio:.6f}")
    
    return zeros_count  # Return zeros count for summation
...
class SelfAttention(Module):
    ...
    def __init__(
        ...
        self.k_proj = MaskedLinear(embed_dim, num_heads * head_dim, bias=True)
        self.v_proj = MaskedLinear(embed_dim, num_heads * head_dim, bias=True)
        self.q_proj = MaskedLinear(embed_dim, num_heads * head_dim, bias=True)
        self.out_proj = MaskedLinear(num_heads * head_dim, embed_dim, bias=True)

        self.register_buffer('w_q_mask', torch.ones(num_heads * head_dim, embed_dim))
        self.register_buffer('w_k_mask', torch.ones(num_heads * head_dim, embed_dim))
        self.register_buffer('w_v_mask', torch.ones(num_heads * head_dim, embed_dim))
        self.register_buffer('w_out_mask', torch.ones(embed_dim, num_heads * head_dim))
        
        
        if prune_heads:
            # self.hard_concrete_for_heads = HardConcrete(n_in=num_heads, init_mean=0.01)
            self.gate_for_q_heads = Gate(n_in=num_heads)
            self.gate_for_k_heads = Gate(n_in=num_heads)
            self.gate_for_v_heads = Gate(n_in=num_heads)
            self.gate_for_out_heads = Gate(n_in=num_heads)
        else:
            self.gate_for_q_heads = None
            self.gate_for_k_heads = None
            self.gate_for_v_heads = None
            self.gate_for_out_heads = None
    ...
    def forward(
        ...
        # w_q_mask = w_k_mask = w_v_mask = w_out_mask = 1
            
        w_q = self.q_proj.linear.weight
        w_k = self.k_proj.linear.weight
        w_v = self.v_proj.linear.weight
        w_out = self.out_proj.linear.weight
        
        if self.gate_for_q_heads is not None:
            self.gate_for_q_heads.get_weight(weight = w_q)
            self.gate_for_k_heads.get_weight(weight = w_k)
            self.gate_for_v_heads.get_weight(weight = w_v)
            self.gate_for_out_heads.get_weight(weight = w_out)
            
            self.w_q_mask = self.gate_for_q_heads()
            self.w_k_mask = self.gate_for_k_heads() 
            self.w_v_mask = self.gate_for_v_heads() 
            self.w_out_mask = self.gate_for_out_heads()
            
            # import pdb;pdb.set_trace()
            # self.q_proj.weight = self.q_proj.weight * w_q_mask
            # self.k_proj.weight = self.k_proj.weight * w_k_mask
            # self.v_proj.weight = self.v_proj.weight * w_v_mask
            # self.out_proj.weight = self.out_proj.weight * w_out_mask
        
        all_masks = [self.w_q_mask, self. w_k_mask, self.w_v_mask, self.w_out_mask]
        mask_names = ["w_q_mask", "w_k_mask", "w_v_mask", "w_out_mask"]
    
        # Print individual statistics and collect zeros count
        
        if self.loop == 0:
            self.total_zeros = 0
            
            for mask, name in zip(all_masks, mask_names):
                zeros_count = print_zero_ratio_and_verify(mask, name)
                self.total_zeros += zeros_count
                
            total_zeros_millions = self.total_zeros / 1e6
            # if (self.loop +1) % 12 == 0:
            print("\n===== Attn Summary =====")
            print(f"Total Pruned Parameters: {total_zeros_millions:.4f}M")
        self.loop = 1
    ...
    def get_num_params(self):
        num_q = self.w_q_mask.sum()
        num_k = self.w_k_mask.sum()
        num_v = self.w_v_mask.sum()
        num_out = self.w_out_mask.sum()
        num_params = num_q + num_k + num_v + num_out + self.q_proj.linear.bias.numel() * 4
        
        return num_params
    
    def get_num_params_stage1(self):
        if self.gate_for_q_heads is not None:
            num_q = self.gate_for_q_heads.num()
            num_k = self.gate_for_k_heads.num()
            num_v = self.gate_for_v_heads.num()
            num_out = self.gate_for_out_heads.num()
            num_params = num_q + num_k + num_v + num_out + self.q_proj.linear.bias.numel() * 4
        else:
            num_params = self.q_proj.weight.numel() + self.k_proj.weight.numel() + self.v_proj.weight.numel() + self.out_proj.weight.numel()
    ...
    # In SelfAttention class
    def prune(self, prune_ratio):
        """
        非结构化修剪 SelfAttention。
        分别对 Q, K, V, Out 矩阵计算 mask。
        """
        # 更新 mask buffer
        # Q projection
        self.w_q_mask.data = compute_unstructured_mask(self.q_proj.linear.weight, prune_ratio)
        
        # K projection
        self.w_k_mask.data = compute_unstructured_mask(self.k_proj.linear.weight, prune_ratio)
        
        # V projection
        self.w_v_mask.data = compute_unstructured_mask(self.v_proj.linear.weight, prune_ratio)
        
        # Out projection
        self.w_out_mask.data = compute_unstructured_mask(self.out_proj.linear.weight, prune_ratio)
        ...
        # 返回配置，指明仍使用 attention 且 head 数量不变
        new_config = {
            "use_attention": True,
...
class WavLMSelfAttention(SelfAttention):
    ...
    def __init__(
        ...
        self.k_proj = MaskedLinear(embed_dim, len(self.remaining_heads) * self.head_dim, bias=bias)
        self.v_proj = MaskedLinear(embed_dim, len(self.remaining_heads) * self.head_dim, bias=bias)
        self.q_proj = MaskedLinear(embed_dim, len(self.remaining_heads) * self.head_dim, bias=bias)
        self.out_proj = MaskedLinear(len(self.remaining_heads) * self.head_dim, embed_dim, bias=bias)
    ...
    def prune(self, prune_ratio):
        """
        非结构化修剪 WavLMSelfAttention。
        与 SelfAttention 类似，但配置中需要返回 remaining_heads。
        """
        # 更新 mask buffer
        self.w_q_mask.data = compute_unstructured_mask(self.q_proj.linear.weight, prune_ratio)
        self.w_k_mask.data = compute_unstructured_mask(self.k_proj.linear.weight, prune_ratio)
...
class FeedForward(Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.intermediate_dense = MaskedLinear(io_features, intermediate_features)
        self.intermediate_dropout = nn.Dropout(intermediate_dropout)
        self.output_dense = MaskedLinear(intermediate_features, io_features)
        self.output_dropout = nn.Dropout(output_dropout)

        self.total_zeros = 0
        self.loop = 0
        
        self.register_buffer('intermediate_mask', torch.ones(intermediate_features, io_features))
        self.register_buffer('output_mask', torch.ones(io_features, intermediate_features))
        
        if prune_intermediate:
            self.gate_for_intermediate = Gate(
                n_in=intermediate_features
            )
            self.gate_for_output = Gate(
                n_in=intermediate_features
            )
        else:
            self.gate_for_intermediate = None
            self.gate_for_output = None
    ...
    def forward(self, x):
        ...
        # intermediate_mask = output_mask = 1
            
        w_in = self.intermediate_dense.linear.weight
        w_out = self.output_dense.linear.weight
        
        if self.gate_for_intermediate is not None:
            self.gate_for_intermediate.get_weight(weight = w_in)
            self.gate_for_output.get_weight(weight = w_out)
            
            self.intermediate_mask = self.gate_for_intermediate()   # 
            self.output_mask = self.gate_for_output()   # 
            
            # self.intermediate_dense.weight = self.intermediate_dense.weight * intermediate_mask
            # self.output_dense.weight = self.output_dense.weight * output_mask
        all_masks = [self.intermediate_mask, self.output_mask]
        mask_names = ["intermediate_mask", "output_mask"]
        ...
        if self.loop == 0:
            ...
            total_zeros_millions = self.total_zeros / 1e6
            # if (self.loop +1) % 12 == 0:
            #     print((self.loop +1) % 12)
            print("\n===== FFN Summary =====")
            print(f"Total Pruned Parameters: {total_zeros_millions:.4f}M")
        self.loop = 1
        
        
        x = self.intermediate_dense(x, weight_mask=self.intermediate_mask)
        x = torch.nn.functional.gelu(x)
        x = self.intermediate_dropout(x)
        
        x = self.output_dense(x, weight_mask=self.output_mask)
    ...
    def get_num_params(self):
        intermediate_features = self.intermediate_mask.sum()
        output_features = self.output_mask.sum()
    ...
    def get_num_params_stage1(self):
        if self.gate_for_intermediate is not None:
            ...
            num_params = self.intermediate_dense.weight.numel() + self.output_dense.weight.numel() 
    ...
    def prune(self, prune_ratio):
        """
        非结构化修剪 FeedForward。
        独立修剪 intermediate_dense 和 output_dense。
        """
        # Intermediate layer (in -> intermediate)
        self.intermediate_mask.data = compute_unstructured_mask(
            self.intermediate_dense.linear.weight, prune_ratio
        )
        
        # Output layer (intermediate -> out)
        self.output_mask.data = compute_unstructured_mask(
            self.output_dense.linear.weight, prune_ratio
        )
        ...
        new_config = {
            ...
            "ff_interm_features": self.intermediate_dense.linear.out_features
        }
...
class Transformer(Module):
    ...
    def prune(self, prune_ratio):
        ...
        for layer in self.layers:
            attention_config = layer.attention.prune(prune_ratio)
            ...
            ff_config = layer.feed_forward.prune(prune_ratio)
...
class Encoder(Module):
    ...
    def prune(self, conv_out_index, prune_ratio):
        ...
        transformer_config = self.transformer.prune(prune_ratio)
