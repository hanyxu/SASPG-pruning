...
class WavLMSelfAttention(SelfAttention):
    ...
    def prune(self):
        ...
        if self.gate_for_q_heads is not None:
            ...
            new_config["remaining_heads"] = self.num_heads
            if new_config["remaining_heads"] == 0:
                new_config["use_attention"] = False
            # else:
            #     full_mask = head_mask.repeat_interleave(self.head_dim)
            #     full_index = full_mask.nonzero().squeeze(-1)  # 1D
...
class FeedForward(Module):
    ...
    def forward(self, x):
        ...
        # Calculate and print total pruned parameters
        # if (self.loop +1) % 12 == 0:
        #     self.total_zeros = 0
        if self.loop == 0:
            self.total_zeros = 0
        ...
        # print_zero_ratio_and_verify(self.intermediate_mask, "intermediate_mask")
        # print_zero_ratio_and_verify(self.output_mask, "output_mask")
