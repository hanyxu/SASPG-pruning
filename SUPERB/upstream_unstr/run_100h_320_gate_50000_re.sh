...
root_dir=exp/hubert-base_${train_subset}_sp${target_sparsity}_spup${sparsity_warmup}_lr${lr}_up${warmup}_max${max}_${distill_mode}${distill_layers}_reglr${reg_lr}_${pruning_units}_gate_norm_320s
...
#     # 检查显存是否大于20000MiB
#     if [ "$mem" -ge 44000 ]; then
#         echo "GPU 2 has sufficient memory (${mem}MiB). Proceeding..."
#         break
#     else
#         echo "Waiting for GPU 2 memory (>44000MiB). Current: ${mem}MiB"
#         sleep 60  # 等待60秒后重试
#     fi
# done
