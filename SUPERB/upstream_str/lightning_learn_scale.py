...
from wav2vec2.model_learn_scale import (
    Wav2Vec2Model,
)
...
class DistillModule(pl.LightningModule):
    def __init__(
        ...
        *,
        ...
        gate_learning_rate: Optional[float],   # lr for loga and lambda
        scale_learning_rate: Optional[float],   # lr for loga and lambda
        tau_learning_rate: Optional[float],   # lr for loga and lambda
        ...
        self.tau_learning_rate = tau_learning_rate
    ...
    def configure_optimizers(self):
        main_params = [p for n, p in self.student_model.named_parameters() if ("gate_threshold" not in n and "tau" not in n and "scale" not in n)]
        # main_params = [p for n, p in self.student_model.named_parameters() if ("gate_threshold" not in n)]
        ...
        if self.use_reg:
            pgs.extend(
                [
                    {
                        'params': [p for n, p in self.student_model.named_parameters() if ("gate_threshold" in n)],
                        # 'lr': self.learning_rate, # -self.reg_learning_rate
                        # 'lr': 5e-5, # -self.reg_learning_rate
                        # 'lr': 4e-4, # -self.reg_learning_rate
                        'lr': self.gate_learning_rate,
                        'weight_decay': 0.0,
                        'name': 'gate_threshold',
                    },
                    {
                        'params': [p for n, p in self.student_model.named_parameters() if ("range_scale" in n)],
                        # 'lr': self.learning_rate, # -self.reg_learning_rate
                        # 'lr': 5e-5, # -self.reg_learning_rate
                        # 'lr': 4e-4, # -self.reg_learning_rate
                    ...
                    {
                        'params': [p for n, p in self.student_model.named_parameters() if "tau" in n],
                        'lr': self.tau_learning_rate,
                        'weight_decay': 0.0,
                        'name': 'tau',
                    },
                    {
