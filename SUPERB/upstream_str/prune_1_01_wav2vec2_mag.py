...
def prune_from_ckpt(distilled_ckpt, original_ckpt):
    ...
    conv_config, use_attention, use_feed_forward, num_heads, remaining_heads, ff_interm_features = model.prune(prune_ratio=args.save_ratio) # 960
    # conv_config, use_attention, use_feed_forward, num_heads, remaining_heads, ff_interm_features = model.prune(prune_ratio=0.1685) # 960
    # conv_config, use_attention, use_feed_forward, num_heads, remaining_heads, ff_interm_features = model.prune(prune_ratio=0.1678) # 100
...
if __name__ == "__main__":
    args = parse_args()
    out_path = args.distilled_ckpt.parent / "pruned_wav2vec2_base.pth"
    torch.save(
        prune_from_ckpt(
