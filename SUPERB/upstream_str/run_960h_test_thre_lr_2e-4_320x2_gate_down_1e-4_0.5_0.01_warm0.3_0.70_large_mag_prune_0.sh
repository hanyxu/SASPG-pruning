...
teacher_ckpt=${SASPG_ROOT}/DPHuBERT_pretrain_unstr/pretrained/hubert-large-ll60k.hf.pth    # checkpoint path
...
final_warmup=0       # warmup steps
final_max=1         # max update steps
...
root_dir=exp_minmax_hubert_large_mag/hubert-large_${train_subset}_sp${target_sparsity}_spup${sparsity_warmup}_lr${lr}_up${warmup}_max${max}_${distill_mode}${distill_layers}_reglr${reg_lr}_gatelr${gate_lr}_${pruning_units}_sec${seconds_per_batch}_accum${accum_grad}_${eta_max}_${eta_min}_mag_finalwrm${final_warmup}_finalmax${final_max}_saveratio${save_ratio}_${final_lr}
...
python prune_1_01_large_mag_0.py \
    --eta_min ${eta_min} \
    --eta_max ${eta_max} \
...
pruned_ckpt=${root_dir}/ckpts/pruned_hubert_large.pth
rm -rf ${root_dir}/ckpts/pruned_hubert_large.pth
cp -r ${SASPG_ROOT}/DPHuBERT_pretrain_unstr/pretrained/pruned_hubert_large.pth ${root_dir}/ckpts/pruned_hubert_large.pth
mkdir -p ${final_exp_dir}
# pruned_ckpt=${SASPG_ROOT}/DPHuBERT_pretrain/exp_minmax_960h_wav2vec2_mag/hubert-large_train960_sp0.7495_spup5000_lr0.0002_up15000_max50000_layer2layer0.4,8,12_reglr0.02_gatelr1e-4_head,interm_sec320_accum2_gate_norm_0.5_0.01_tau_gate_up_30%_down_mag/lr0.0001_up15000_max75000/ckpts/epoch=12-step=59501.ckpt
...
python save_final_ckpt_large.py \
