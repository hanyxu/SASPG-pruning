...
warmup=15            # warmup steps
max=25               # max update steps
...
sparsity_warmup=5    # warmup steps for sparsity; sparsity will linearly increase from 0 to target
seconds_per_batch=320
accum_grad=2
eta_max=0.5
eta_min=0.01

root_dir=exp_minmax_split_new/hubert-base_${train_subset}_sp${target_sparsity}_spup${sparsity_warmup}_lr${lr}_up${warmup}_max${max}_${distill_mode}${distill_layers}_reglr${reg_lr}_gatelr${gate_lr}_${pruning_units}_sec${seconds_per_batch}_accum${accum_grad}_gate_norm_${eta_max}_${eta_min}_tau_gate_up_30%_down_re_split_new_forward_from_start_test

# 之前是10 tau 0.5-> 0.01 50000tausteps
# final distill config
final_lr=0.0001         # learning rate for final distillation (training step 2)
final_warmup=5       # warmup steps
final_max=25         # max update steps
...
#     --sparsity_warmup_updates ${sparsity_warmup} 
    # 2>&1 | tee ${root_dir}/distill.log || exit 1;

# prune and save model
# python prune_1_01_split_last.py \
#     --eta_min ${eta_min} \
#     --eta_max ${eta_max} \
#     --distilled_ckpt ${SASPG_ROOT}/DPHuBERT_pretrain/exp_minmax_split/hubert-base_train100_sp0.75_spup5000_lr0.0002_up15000_max50000_layer2layer0.4,8,12_reglr0.02_gatelr1e-4_head,interm_sec320_accum2_gate_norm_0.5_0.01_tau_gate_up_30%_down_re_split_new_forward/lr0.0001_up5000_max25000/ckpts/epoch=52-step=25000.ckpt \
...
# pruned_ckpt=${root_dir}/lr0.0001_up5000_max25000/ckpts/pruned_hubert_base_split_save.pth
pruned_ckpt=${root_dir}/ckpts/pruned_hubert_base_stage1_split_save.pth

# mkdir -p ${final_exp_dir}

# python final_distill_return_from_split.py \
#     --tsv_dir ${tsv_dir} \
#     --train_subset ${train_subset} \
#     --seconds_per_batch ${seconds_per_batch} \
#     --num_workers 12 \
...
python save_final_ckpt_split_last.py \
