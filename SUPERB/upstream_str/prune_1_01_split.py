...
def prune_from_ckpt(distilled_ckpt, original_ckpt):
    ...
    conv_config, use_attention, use_feed_forward, num_heads, remaining_qk_heads, remaining_vo_heads, ff_interm_features = model.prune()
    
    
    # import pdb;pdb.set_trace()
    pruned_config = config.copy()
    if len(num_heads) == 0:     # for wavlm
        assert len(remaining_vo_heads) > 0 and len(remaining_vo_heads) > 0 # here > 0 means it is not empty, i.e. []
        pruned_config.update(
            {
                "encoder_remaining_qk_heads": remaining_qk_heads,
                "encoder_remaining_vo_heads": remaining_vo_heads,
            }
        )
    else:
        pruned_config.update(
    ...
    pruned_config.update(
        {
            ...
            # "encoder_prune_qk_heads": False,
            # "encoder_prune_vo_heads": False,
...
def load_pruned_model(ckpt_path, attn_mask):
