...
root_dir=exp/hubert-base_${train_subset}_sp${target_sparsity}_spup${sparsity_warmup}_lr${lr}_up${warmup}_max${max}_${distill_mode}${distill_layers}_reglr${reg_lr}_gatelr${gate_lr}_${pruning_units}_sec${seconds_per_batch}_accum${accum_grad}_gate_norm_0.5_0.01_tau_gate_up_15%_down

# 之前是10 tau 0.5-> 0.01 50000tausteps
# final distill config
final_lr=0.0001         # learning rate for final distillation (training step 2)
final_warmup=10000       # warmup steps
final_max=50000         # max update steps
