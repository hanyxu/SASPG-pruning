...
root_dir=exp_minmax/hubert-base_${train_subset}_sp${target_sparsity}_spup${sparsity_warmup}_lr${lr}_up${warmup}_max${max}_${distill_mode}${distill_layers}_reglr${reg_lr}_gatelr${gate_lr}_${pruning_units}_sec${seconds_per_batch}_accum${accum_grad}_gate_norm_${eta_max}_${eta_min}_tau_gate_up_15%_down_reverse
...
python final_distill.py \
    ...
    --l1_weight ${l1_weight} \
    --cos_weight ${cos_weight} \
    --cos_type ${cos_type} 2>&1 | tee ${final_exp_dir}/final_distill_prune_1_01_true.log || exit 1;
