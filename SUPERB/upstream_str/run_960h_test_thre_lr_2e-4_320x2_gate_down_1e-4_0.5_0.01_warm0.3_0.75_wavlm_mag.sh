...
teacher_ckpt=${DPHuBERT_PRETRAINED_ROOT}/wavlm-base-plus.hf.pth    # checkpoint path
...
# 定义要使用的训练子集名称（可根据需要修改这里）

if [ "$train_subset" = "train960" ]; then
    save_ratio=0.1677 # 23604372
elif [ "$train_subset" = "train100" ]; then
    save_ratio=0.1675 # 23585928
else
...
root_dir=exp_minmax_wavlm_base_mag/wavlm-base_${train_subset}_sp${target_sparsity}_sec${seconds_per_batch}_accum${accum_grad}_${eta_max}_${eta_min}_mag_finalwrm${final_warmup}_finalmax${final_max}_saveratio${save_ratio}_${final_lr}
...
python prune_1_01_wavlm_mag.py \
    --eta_min ${eta_min} \
    --eta_max ${eta_max} \
...
pruned_ckpt=${root_dir}/ckpts/pruned_wavlm_base.pth
rm -rf ${root_dir}/ckpts/pruned_wavlm_base.pth
cp -r "${DPHuBERT_PRETRAINED_ROOT}/pruned_wavlm_base.pth" ${root_dir}/ckpts/pruned_wavlm_base.pth
...
python save_final_ckpt_wavlm.py \
    --config_path ${pruned_ckpt} \
    --ckpt_after_final_distill ${final_exp_dir}/ckpts/*.ckpt || exit 1;


# python3 run_downstream.py -n test -m train -u dphubert -d fluent_commands -k ${SASPG_ROOT}/DPHuBERT_pretrain/exp_minmax_wavlm_base_mag/wavlm-base_train960_sp0.75_spup5000_lr0.0002_up15000_max50000_layer2layer0.4,8,12_reglr0.02_gatelr1e-4_head,interm_sec320_accum2_0.5_0.01_mag_finalwrm5000_finalmax25000_saveratio0.1677_0.0001/ckpts/pruned_wavlm_base.pth
# python3 run_downstream.py -n test -m train -u dphubert -d fluent_commands -k ${SASPG_ROOT}/DPHuBERT_pretrain/exp_minmax_wavlm_base_mag/wavlm-base_train100_sp0.75_spup5000_lr0.0002_up15000_max50000_layer2layer0.4,8,12_reglr0.02_gatelr1e-4_head,interm_sec320_accum2_0.5_0.01_mag_finalwrm5000_finalmax25000_saveratio0.1675_0.0001/ckpts/pruned_wavlm_base.pth
