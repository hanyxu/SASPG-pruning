...
warmup=7500            # warmup steps
max=25000               # max update steps
pruning_units=head,interm,conv      # conv,head,interm,attlayer,ffnlayer
reg_lr=0.02             # learning rate for regularization params
gate_lr=2e-4             
target_sparsity=0.75    # final target sparsity
sparsity_warmup=2500    # warmup steps for sparsity; sparsity will linearly increase from 0 to target
seconds_per_batch=320
accum_grad=2

root_dir=exp/hubert-base_${train_subset}_sp${target_sparsity}_spup${sparsity_warmup}_lr${lr}_up${warmup}_max${max}_${distill_mode}${distill_layers}_reglr${reg_lr}_gatelr${gate_lr}_${pruning_units}_sec${seconds_per_batch}_accum${accum_grad}_gate_norm_0.5_0.01_tau_1e-5_gate_init_prune_conv

# 之前是10 tau 0.5-> 0.01 50000tausteps
# final distill config
final_lr=0.0001         # learning rate for final distillation (training step 2)
final_warmup=2500       # warmup steps
final_max=12500         # max update steps
