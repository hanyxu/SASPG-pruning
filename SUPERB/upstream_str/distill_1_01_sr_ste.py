...
from wav2vec2.model_1_01_sr_ste import (
    wav2vec2_model,
)
from wav2vec2.gate_1_01_sr_ste import Gate

_LG = logging.getLogger(f"{__name__}:{_get_rank()}")


def set_gate_max_steps(model, max_steps=None):
    def apply_max_steps(module):
        if hasattr(module, 'total_steps'):
            module.total_steps = max_steps
    
    for name, module in model.named_modules():
        if isinstance(module, Gate):
            apply_max_steps(module)
...
def set_init_sparsity(model, init_sparsity=None):
    def apply_init_sparsity(module):
        if hasattr(module, 'init_sparsity'):
            module.init_sparsity = init_sparsity
    
    for name, module in model.named_modules():
        apply_init_sparsity(module)
...
def run_train(args):
    ...
    student_model = set_gate_max_steps(student_model, max_steps=args.max_updates * int(args.accum_grad))
    student_model = set_tau_min_max(student_model, eta_min=args.eta_min, eta_max=args.eta_max)
    student_model = set_tau_reverse(student_model, reverse_tau=args.reverse_tau)
    student_model = set_shrink_scale(student_model, shrink_scale=args.shrink_scale)
    student_model = set_init_sparsity(student_model, init_sparsity=args.init_sparsity)
    ...
    distill_module = DistillModule(
        ...
        tau_learning_rate=args.tau_learning_rate,
