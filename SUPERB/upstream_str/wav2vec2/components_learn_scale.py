...
#     prune_layer_norm,
# )
from .gate_learn_scale import Gate
from .pruning_utils import (
    prune_linear_layer,
    prune_conv1d_layer,
    prune_layer_norm,
...
class SelfAttention(Module):
    ...
    def __init__(
        ...
        self.log_scale = nn.Parameter(
            torch.log(torch.tensor(initial_scale, dtype=torch.float32)),
            requires_grad=True
        )
        self.range_scale = nn.Parameter(
            torch.tensor(initial_scale_1, dtype=torch.float32),
            requires_grad=True
        )
        ...
        if prune_heads:
            ...
            self.gate_for_heads = Gate(n_in=num_heads)
        else:
    ...
    def normalize_minmax(self, sum_score: torch.Tensor) -> torch.Tensor:
        ...
        if not self.reset_scale and self.training:
            device = self.log_scale.device

            new_value = torch.log(max_val - min_val).clone().detach().to(device)
            
            self.log_scale.data.copy_(new_value)

            if self.log_scale.grad is not None:
                self.log_scale.grad.zero_() 
            
            print('minus scale set attn:', 1.0 / (max_val - min_val))
            self.reset_scale = True
            tmp_scale = torch.exp(self.log_scale)
            
            print('minus tmp_scale * sum_score.min():', (sum_score - min_val) / tmp_scale + 1e-5)
        
        # import pdb;pdb.set_trace()
        
        scale = torch.exp(self.log_scale)
        
        normalized = (sum_score - min_val) / scale + 1e-5
    ...
    def normalize_minmax_1(self, sum_score: torch.Tensor) -> torch.Tensor:
        ...
        scale = torch.exp(self.range_scale)
        
        # normalized = (sum_score - min_val) / scale + 1e-5
        normalized =scale* (sum_score - min_val) / ((max_val - min_val)) + 1e-5
        # print('normalized ffn',normalized)
        # normalized = 32 * sum_score / sum_score.sum()
        # normalized = sum_score
        # normalized = 100*sum_score / sum_score.sum()
        if self.training:
            if self.current_steps % 1000 == 0:
                print('ffn self.current_steps:', self.current_steps)
                print('ffn scale:', scale)
            self.current_steps += 1
            
        return normalized
    
    def normalize_scale(self, sum_score: torch.Tensor) -> torch.Tensor:
        
        if not self.reset_scale and self.training:
        # if not self.reset_scale:
            device = self.log_scale.device

            new_value = torch.log( 1.0 / (sum_score.max() + 1e-8)).clone().detach().to(device)
            ...
            print('scale set attn:', 1.0 / (sum_score.max() + 1e-8))
            self.reset_scale = True
            tmp_scale = torch.exp(self.log_scale)
            
            print('tmp_scale * sum_score.min():', tmp_scale * sum_score.min())
                
        scale = torch.exp(self.log_scale)
        sum_score = scale * sum_score
        if self.training:
            if self.current_steps % 1000 == 0:
                print('attn self.current_steps:', self.current_steps)
                print('attn scale:', scale)
            self.current_steps += 1
        return sum_score
    ...
    def forward(
        ...
        """
        Args:
            x (Tensor): shape: ``[ba m ch_size, sequence_length, embed_dim]``.
        ...
        if self.gate_for_heads is not None:
            if self.norm_type == 'minmax':
                self.gate_for_heads.get_channel_score(channel_score = self.normalize_minmax(channel_score))
            elif self.norm_type == 'minmax_1':
                self.gate_for_heads.get_channel_score(channel_score = self.normalize_minmax_1(channel_score))
            elif self.norm_type == 'scale':
                self.gate_for_heads.get_channel_score(channel_score = self.normalize_scale(channel_score))
            else:
                raise NotImplementedError()
            # self.gate_for_heads.get_channel_score(channel_score = self.normalize_0_1(channel_score))
    ...
    def get_num_params(self):
        if self.gate_for_heads is not None:
            num_heads = self.gate_for_heads.num()
        else:
...
class FeedForward(Module):
    ...
    def normalize_minmax(self, sum_score: torch.Tensor) -> torch.Tensor:
        ...
        if not self.reset_scale and self.training:
            ...
            print('minus scale set ffn:', 1.0 / (max_val - min_val))
            self.reset_scale = True
            tmp_scale = torch.exp(self.log_scale)
        ...
        normalized = (sum_score - min_val) / scale + 1e-5
        # normalized = (sum_score - min_val) / (scale*(max_val - min_val)) + 1e-5
        # print('normalized ffn',normalized)
        # normalized = 32 * sum_score / sum_score.sum()
        # normalized = sum_score
        # normalized = 100*sum_score / sum_score.sum()
    ...
    def normalize_scale(self, sum_score: torch.Tensor) -> torch.Tensor:
        ...
        # if not self.reset_scale:
            ...
            print('scale set ffn:', 1.0 / (sum_score.max() + 1e-8))
            self.reset_scale = True
            tmp_scale = torch.exp(self.log_scale)
    ...
    def forward(self, x):
        ...
        if self.gate_for_intermediate is not None:
            if self.norm_type == 'minmax':
                self.gate_for_intermediate.get_channel_score(channel_score = self.normalize_minmax(channel_score))
            elif self.norm_type == 'minmax_1':
                self.gate_for_intermediate.get_channel_score(channel_score = self.normalize_minmax_1(channel_score))
            elif self.norm_type == 'scale':
                self.gate_for_intermediate.get_channel_score(channel_score = self.normalize_scale(channel_score))
            else:
                raise NotImplementedError()
            # self.gate_for_intermediate.get_channel_score(channel_score = self.normalize(channel_score))
            # self.gate_for_intermediate.get_channel_score(channel_score = self.normalize_0_1(channel_score))
