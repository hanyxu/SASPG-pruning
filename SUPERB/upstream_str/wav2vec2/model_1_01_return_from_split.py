...
from . import components_return_from_split as components
# import components
...
class Wav2Vec2Model(Module):
    ...
    def extract_features(
        ...
        # print(type(waveforms), waveforms, len(waveforms))
        if len(waveforms) == 1:     
            len(waveforms[0].shape) == 1
            waveforms = waveforms[0].unsqueeze(0)
        else:
            assert len(waveforms[0].shape) == 1
    ...
    #     remaining_qk_heads = transformer_config["remaining_qk_heads"]
    #     remaining_vo_heads = transformer_config["remaining_vo_heads"]
    #     ff_interm_features = transformer_config["ff_interm_features"]

    #     return conv_config, use_attention, use_feed_forward, num_heads, remaining_qk_heads, remaining_vo_heads, ff_interm_features
...
class Wav2Vec2Model_split_save(Module):
    """Acoustic model used in *wav2vec 2.0* :cite:`baevski2020wav2vec`.
    ...
    def prune_split_save(self):
        self.eval()     # must be in eval mode
        conv_config, conv_out_index = self.feature_extractor.prune()    # [(output_channel, kernel_size, stride), ...]
        transformer_config = self.encoder.prune_split_save(conv_out_index)     # NOTE: this is a defaultdict(list)
        use_attention = transformer_config["use_attention"]
        use_feed_forward = transformer_config["use_feed_forward"]
        num_heads = transformer_config["num_heads"]     # can be []
        remaining_heads = transformer_config["remaining_heads"]     # can be []
...
def wav2vec2_model_split_save(**configs) -> Wav2Vec2Model:
    """Wraps the original wav2vec2_model and wavlm_model."""

    if "encoder_remaining_heads" in configs:
        return wavlm_model_split_save(**configs)
    
    return wav2vec2_model_original_split_save(**configs)


def wav2vec2_model_split_half(**configs) -> Wav2Vec2Model:
    """Wraps the original wav2vec2_model and wavlm_model."""

    if "encoder_remaining_heads" in configs:
        return wavlm_model_split_half(**configs)
    
    return wav2vec2_model_original_split_half(**configs)
...
def wav2vec2_model_original_split_save(
    extractor_mode: str,
    extractor_conv_layer_config: Optional[List[Tuple[int, int, int]]],
    extractor_conv_bias: bool,
    encoder_embed_dim: int,
    ...
    encoder = components._get_encoder_split_save(
        in_features=extractor_conv_layer_config[-1][0],
        embed_dim=encoder_embed_dim,
        dropout_input=encoder_projection_dropout,
        pos_conv_kernel=encoder_pos_conv_kernel,
    ...
    if aux_num_out is not None:
        aux = torch.nn.Linear(in_features=encoder_embed_dim, out_features=aux_num_out)
    return Wav2Vec2Model_split_save(normalize_waveform, feature_extractor, encoder, aux)
