...
class SelfAttention(Module):
    """Multihead Self Attention module
    for teacher only in downstream task
    Args:
        embed_dim (int): Total dimension of the model.
        num_heads (int): The number of heads.
        dropout (float, optional):
    ...
    def forward(
        ...
        # channel_score = (w_q**2).sum(dim=1) + (w_k**2).sum(dim=1) + (w_v**2).sum(dim=1) + (w_out**2).sum(dim=0)
        # channel_score = channel_score.reshape(self.num_heads, -1, 1).sum(dim=1)
        
        # if self.gate_for_heads is not None:
        #     self.gate_for_heads.get_channel_score(channel_score = self.normalize(channel_score))
        #     head_mask = self.gate_for_heads().squeeze(-1)  # (nH,)
        #     # import pdb;pdb.set_trace()
