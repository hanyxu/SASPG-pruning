...
# class SelfAttention_split_save(Module):
...
#         self.final_distill = False
        
#         self.qk_unmask_num = 1
#         self.vo_unmask_num = 1
#         self.embed_dim = embed_dim
...
#         self.q_proj = nn.Linear(embed_dim, num_heads['num_qk'], bias=True)
#         self.k_proj = nn.Linear(embed_dim, num_heads['num_qk'], bias=True)
#         self.v_proj = nn.Linear(embed_dim, num_heads['num_vo'], bias=True)
#         self.out_proj = nn.Linear(num_heads['num_vo'], embed_dim, bias=True)
        
#         self.gate_for_qk = Gate(n_in=num_heads * head_dim)
#         self.gate_for_vo = Gate(n_in=num_heads * head_dim)
...
#             self.gate_for_vo = Gate(n_in=num_heads * head_dim)
#         else:
#             self.gate_for_qk = None
#             self.gate_for_vo = None
...
#         # # print('normalized ffn',normalized)
#         # normalized = 32 * sum_score / sum_score.sum()
#         # normalized = sum_score
#         # normalized = 100*sum_score / sum_score.sum()
...
#         qk_channel_score = (w_q**2).sum(dim=1) + (w_k**2).sum(dim=1)
#         vo_channel_score = (w_v**2).sum(dim=1) + (w_out**2).sum(dim=0)
        
#         # 初始化掩码
#         qk_mask = None
#         vo_mask = None
        
#         # Final distill 模式下的掩码设置
#         if self.final_distill:
#             # 首次训练时设置固定掩码
#             if self.training and not hasattr(self, '_qk_mask_initialized'):
#                 # 设置qk_mask
#                 _, top_qk_indices = torch.topk(qk_channel_score, k=self.qk_unmask_num)
#                 qk_mask = torch.zeros(embed_dim, device=x.device)
#                 qk_mask[top_qk_indices] = 1.0
#                 print('self.qk_unmask_num:', self.qk_unmask_num)
#                 self.register_buffer('_fixed_qk_mask', qk_mask)
#                 self._qk_mask_initialized = True
                
#                 # 设置vo_mask
#                 _, top_vo_indices = torch.topk(vo_channel_score, k=self.vo_unmask_num)
#                 vo_mask = torch.zeros(embed_dim, device=x.device)
#                 vo_mask[top_vo_indices] = 1.0
#                 print('self.vo_unmask_num:', self.vo_unmask_num)
#                 self.register_buffer('_fixed_vo_mask', vo_mask)
#                 self._vo_mask_initialized = True

#                 # 计算原始参数量（未修剪时）
#                 original_params = (self.embed_dim * self.embed_dim + self.embed_dim) * 2 + \
#                                 (self.embed_dim * self.embed_dim + self.embed_dim) + \
#                                 (self.embed_dim * self.embed_dim + self.embed_dim)

#                 # 获取当前保留的参数量
#                 remaining_params = self.get_num_params()

#                 # 计算修剪掉的参数量
#                 pruned_params = original_params - remaining_params

#                 print('Attn param (M) saved / Attn param pruned:', remaining_params / 1e6, pruned_params/1e6)
                
#             # 使用固定掩码
#             if hasattr(self, '_fixed_qk_mask'):
#                 qk_mask = self._fixed_qk_mask
#             if hasattr(self, '_fixed_vo_mask'):
#                 vo_mask = self._fixed_vo_mask
#         else:
#             # 非final distill模式下的动态掩码
#             if self.gate_for_qk is not None:
#                 self.gate_for_qk.get_channel_score(channel_score = self.normalize(qk_channel_score))
#                 qk_mask = self.gate_for_qk().squeeze(-1)  # (embed_dim,)
                
#             if self.gate_for_vo is not None:
#                 self.gate_for_vo.get_channel_score(channel_score = self.normalize(vo_channel_score))
#                 vo_mask = self.gate_for_vo().squeeze(-1)  # (embed_dim,)
        
#         # 应用qk_mask到输入
#         if qk_mask is not None:
#             x_qk = x * qk_mask.unsqueeze(0).unsqueeze(0)  # 应用qk通道掩码
#         else:
#             x_qk = x
            
#         # 应用vo_mask到值投影后的输出
#         apply_vo_mask_after_v_proj = (vo_mask is not None)
        
#         shape = (batch_size, length, self.num_heads, self.head_dim)
#         q = self.q_proj(x_qk).view(*shape).transpose(2, 1)  # B, nH, L, Hd
#         k = self.k_proj(x_qk).view(*shape).permute(0, 2, 3, 1)  # B, nH, Hd, L
...
#         # 应用vo_mask
#         if apply_vo_mask_after_v_proj:
#             output = output * vo_mask.unsqueeze(0).unsqueeze(0)
...
#         io_dim = self.embed_dim

#         if self.gate_for_qk is not None:
#             num_qk = self.gate_for_qk.num()
#         else:
#             num_qk = self.num_heads * self.head_dim
            
#         num_params_qk = (io_dim + 1) * num_qk * 2
#         # # # print('num_params_qk',num_params_qk)
        
#         if self.gate_for_vo is not None:
#             num_vo = self.gate_for_vo.num()
#         else:
#             num_vo = self.num_heads * self.head_dim
            
#         num_params_vo = (io_dim + 1) * num_vo + (num_vo + 1) * io_dim
        

#         # if self.hard_concrete_for_layer is not None:
#         #     num_params *= self.hard_concrete_for_layer.l0_norm()
#         num_params = num_params_qk + num_params_vo
#         return num_params"""
...
#         # 计算qk投影的参数量
#         if hasattr(self, '_fixed_qk_mask'):  # final_distill模式下使用固定掩码
#             num_qk = int(self._fixed_qk_mask.sum().item())
#         elif self.gate_for_qk is not None:
#             num_qk = self.gate_for_qk.num()  # 假设存在此方法
#         else:
#             num_qk = io_dim  # 如果没有剪枝，使用全量通道
            
#         # q_proj 和 k_proj 的参数量: (输入维度 × 输出维度) + 偏置
#         num_params_qk = (io_dim * num_qk + num_qk) * 2  # ×2 是因为有q和k两个投影
        
#         # 计算vo投影的参数量
#         if hasattr(self, '_fixed_vo_mask'):  # final_distill模式下使用固定掩码
#             num_vo = int(self._fixed_vo_mask.sum().item())
#         elif self.gate_for_vo is not None:
#             num_vo = self.gate_for_vo.num()  # 假设存在此方法
#         else:
#             num_vo = io_dim  # 如果没有剪枝，使用全量通道
            
#         # v_proj 和 out_proj 的参数量
#         num_params_vo = (io_dim * num_vo + num_vo) + (num_vo * io_dim + io_dim)
        
#         num_params = num_params_qk + num_params_vo
#         return num_params

#     def prune_split_save(self):
#         """使用固定掩码对模型进行原地修剪，并返回新配置"""
#         new_config = {
#             "use_attention": True,
#             "num_heads": self.num_heads,
#         }
        
#         # 确保已设置final_distill模式
#         if not self.final_distill or not hasattr(self, '_fixed_qk_mask'):
#             raise ValueError("请先调用set_final_distill()设置固定掩码")
        
#         # 获取保留的通道索引
#         qk_keep_indices = torch.nonzero(self._fixed_qk_mask, as_tuple=True)[0]
#         vo_keep_indices = torch.nonzero(self._fixed_vo_mask, as_tuple=True)[0]
        
#         # 更新配置中的通道数
#         num_head_dict = {
#             'num_qk': len(qk_keep_indices),
#             'num_vo': len(vo_keep_indices),
#         }
#         new_config["num_heads"] = num_head_dict
        
#         # 如果vo通道数为0，则禁用注意力层
#         if num_head_dict["num_vo"] == 0:
#             new_config["use_attention"] = False
#         else:
#             # 修剪Q和K投影层（输出维度）
#             prune_linear_layer(self.q_proj, qk_keep_indices, "output")
#             prune_linear_layer(self.k_proj, qk_keep_indices, "output")
            
#             # 修剪V投影层（输出维度）
#             prune_linear_layer(self.v_proj, vo_keep_indices, "output")
            
#             # 修剪输出投影层（输入维度）
#             prune_linear_layer(self.out_proj, vo_keep_indices, "input")
        
#         # 删除不再需要的门控机制
#         self.gate_for_qk_channels = None
#         self.gate_for_vo_channels = None
        
#         # 更新embed_dim为修剪后的维度
#         self.embed_dim = len(vo_keep_indices)
        
#         print(f"[Model Pruned] 通道数从{self.original_qk_dim}→{self.embed_dim}")
#         print(f"保留通道数：qk={len(qk_keep_indices)}, vo={len(vo_keep_indices)}")
...
#         return new_config
    ...
    #     channel_score_qk = (w_q**2).sum(dim=1) + (w_k**2).sum(dim=1) 
        
    #     channel_score_vo = (w_v**2).sum(dim=1) + (w_out**2).sum(dim=0)
        
    #     if self.gate_for_qk is not None:
    #         assert not self.gate_for_qk.training
    #         self.gate_for_qk.get_channel_score(channel_score = self.normalize(channel_score_qk))
    #         qk_mask = self.gate_for_qk() 
    #         # import pdb;pdb.set_trace()
    #         num_head_dict = {}
    #         num_head_dict['num_qk'] = len(qk_mask.nonzero())
            
    #         assert not self.gate_for_vo.training
    #         self.gate_for_vo.get_channel_score(channel_score = self.normalize(channel_score_vo))
    #         vo_mask = self.gate_for_vo()  
    #         # import pdb;pdb.set_trace()
    #         num_head_dict["num_vo"] = len(vo_mask.nonzero())
            
    #         new_config["num_heads"] = num_head_dict
            
    #         if new_config["num_heads"]["num_vo"] == 0:
    #         # if new_config["num_heads"] == 0:
    #             new_config["use_attention"] = False
    #         # else:
    #         #     full_index_qk = qk_mask.nonzero().squeeze(-1)  # 1D
    #         #     full_index_vo = vo_mask.nonzero().squeeze(-1)  # 1D

    #         #     prune_linear_layer(self.k_proj, full_index_qk, "output")
    #         #     prune_linear_layer(self.q_proj, full_index_qk, "output")
    #         #     prune_linear_layer(self.v_proj, full_index_vo, "output")

    #         #     self.out_proj.weight.data *= vo_mask
    #         #     prune_linear_layer(self.out_proj, full_index_vo, "input")
                    
    #         self.gate_for_qk = None
    #         self.gate_for_vo = None
    #         # self.qk_mask = qk_mask
    #         # self.vo_mask = vo_mask
    #     # # print('num_head_dict:',num_head_dict)
    #     # print('attn-new_config', new_config)
    #     return new_config
...
class SelfAttention(Module):
    ...
    def __init__(
        ...
        # elif isinstance(num_heads, dict):
        #     self.q_proj = nn.Linear(embed_dim, num_heads['num_qk'], bias=True)
        #     self.k_proj = nn.Linear(embed_dim, num_heads['num_qk'], bias=True)
        #     self.v_proj = nn.Linear(embed_dim, num_heads['num_vo'], bias=True)
        #     self.out_proj = nn.Linear(num_heads['num_vo'], embed_dim, bias=True)

        # self.gate_for_qk = Gate(n_in=num_heads * head_dim)  # For query-key pathway
        # self.gate_for_vo = Gate(n_in=num_heads * head_dim)  # For value-output pathway

        # Gating mechanisms for channels
        if prune_heads:
            self.gate_for_qk = Gate(n_in=num_heads * head_dim)  # For query-key pathway
            self.gate_for_vo = Gate(n_in=num_heads * head_dim)  # For value-output pathway
        else:
        ...
        # Buffers to store channel masks
        self.register_buffer('qk_mask', torch.ones(num_heads * head_dim))
        self.register_buffer('vo_mask', torch.ones(num_heads * head_dim))
    ...
    def forward(
        ...
        # Compute channel scores directly from weight matrices
        if self.gate_for_qk is not None:
            w_q = self.q_proj.weight
            w_k = self.k_proj.weight
            w_v = self.v_proj.weight
            ...
            self.gate_for_qk.get_channel_score(channel_score = self.normalize(qk_channel_score))
            self.qk_mask = self.gate_for_qk()

            self.gate_for_vo.get_channel_score(channel_score = self.normalize(vo_channel_score))
            self.vo_mask = self.gate_for_vo()  # (embed_dim,)

            # import pdb;pdb.set_trace()
            # Assert that masks contain only binary values (0 or 1)
            mask_values = torch.unique(torch.cat([self.qk_mask, self.vo_mask]))
            assert torch.all(mask_values == torch.tensor([0.0, 1.0], device=mask_values.device)) or \
                   torch.all(mask_values == torch.tensor([0.0], device=mask_values.device)) or \
                   torch.all(mask_values == torch.tensor([1.0], device=mask_values.device)), \
                   "qk_mask and vo_mask must contain only binary values (0 or 1)"
        
        # Project inputs
        q = self.q_proj(x)  # [B, L, nH*Hd]
        k = self.k_proj(x)  # [B, L, nH*Hd]
        v = self.v_proj(x)  # [B, L, nH*Hd]
        
        # Apply channel masks
        if self.gate_for_qk is not None:
            q_pruned = q * self.qk_mask.unsqueeze(0).unsqueeze(0)  # [B, L, nH*Hd]
            k_pruned = k * self.qk_mask.unsqueeze(0).unsqueeze(0)  # [B, L, nH*Hd]
            v_pruned = v * self.vo_mask.unsqueeze(0).unsqueeze(0)  # [B, L, nH*Hd]
            # import pdb;pdb.set_trace()
            
            # Ensure q and k have identical pruning patterns
            q_zero_channels = (self.qk_mask == 0)
            k_zero_channels = (self.qk_mask == 0)
            assert torch.all(q_zero_channels == k_zero_channels), "Pruned channels in q and k must be identical"
            
            # Reshape for multi-head attention
            q_reshaped = q_pruned.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)  # [B, nH, L, Hd]
            k_reshaped = k_pruned.view(batch_size, seq_len, self.num_heads, self.head_dim).permute(0, 2, 3, 1)  # [B, nH, Hd, L]
            v_reshaped = v_pruned.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)  # [B, nH, L, Hd]
        else:
            # If no pruning, use original values
            q_reshaped = q.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)  # [B, nH, L, Hd]
            k_reshaped = k.view(batch_size, seq_len, self.num_heads, self.head_dim).permute(0, 2, 3, 1)  # [B, nH, Hd, L]
            v_reshaped = v.view(batch_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)  # [B, nH, L, Hd]
        
        # Compute scaled dot-product attention
        attn_weights = (self.scaling * q_reshaped) @ k_reshaped  # [B, nH, L, L]
        ...
        attn_output = attn_weights @ v_reshaped  # [B, nH, L, Hd]
        attn_output = attn_output.transpose(1, 2).reshape(batch_size, seq_len, self.num_heads * self.head_dim)
        
        # Apply output channel mask
        if self.gate_for_qk is not None:
            attn_output_pruned = attn_output * self.vo_mask.unsqueeze(0).unsqueeze(0)
            # import pdb;pdb.set_trace()
            # Ensure v and output have identical pruning patterns
            v_zero_channels = (self.vo_mask == 0)
            output_zero_channels = (self.vo_mask == 0)
            assert torch.all(v_zero_channels == output_zero_channels), "Pruned channels in v and output must be identical"
        else:
            attn_output_pruned = attn_output
        
        # Final projection
        output = self.out_proj(attn_output_pruned)
    ...
    #     # 确保已设置final_distill模式
    #     if not hasattr(self, '_fixed_qk_mask'):
    #         raise ValueError("请先调用set_final_distill()设置固定掩码")
...
class WavLMSelfAttention(SelfAttention):
    ...
    def __init__(
        ...
        remaining_qk_heads: Optional[List[int]] = None,
        remaining_vo_heads: Optional[List[int]] = None,
        dropout: float = 0.0,
        bias: bool = True,
        ...
        if remaining_qk_heads is None and remaining_heads is None:
            assert remaining_vo_heads is None
            self.qkvo = False
            self.remaining_heads = list(range(total_num_heads))
        else:
            self.remaining_qk_heads = remaining_qk_heads  # list of indices
            self.remaining_vo_heads = remaining_vo_heads  # list of indices
            
        
        self.head_dim = embed_dim // total_num_heads

        super().__init__(embed_dim, total_num_heads, self.head_dim, dropout, prune_heads, prune_layer)
        ...
        self.k_proj = nn.Linear(embed_dim, len(self.remaining_qk_heads), bias=bias)
        self.q_proj = nn.Linear(embed_dim, len(self.remaining_qk_heads), bias=bias)
        self.v_proj = nn.Linear(embed_dim, len(self.remaining_vo_heads), bias=bias)
        self.out_proj = nn.Linear(len(self.remaining_vo_heads), embed_dim, bias=bias)

        self.gru_rel_pos = gru_rel_pos
        if self.gru_rel_pos:
            self.gru_rel_pos_linear = nn.Linear(self.head_dim, 8)
            self.gru_rel_pos_const = nn.Parameter(torch.ones(1, total_num_qk_heads, 1, 1))
    ...
    def forward(
        ...
        if self.rel_attn_embed is not None and position_bias is None:
            ...
            position_bias = position_bias.unsqueeze(0).repeat(bsz, 1, 1, 1).view(bsz * self.total_num_qk_heads, seq_len, seq_len)

        attn_mask_rel_pos: Optional[Tensor] = None
        if position_bias is not None:
            attn_mask_rel_pos = position_bias
            if self.gru_rel_pos:  # Apply gating on relative position bias
                query_layer = query.view(bsz, seq_len, self.total_num_qk_heads, -1)
                query_layer = query_layer.permute(0, 2, 1, 3)

                gate_a, gate_b = torch.sigmoid(
                    self.gru_rel_pos_linear(query_layer).view(bsz, self.total_num_qk_heads, seq_len, 2, 4).sum(-1, keepdim=False)
                ).chunk(2, dim=-1)
                gate_a_1 = gate_a * (gate_b * self.gru_rel_pos_const - 1.0) + 2.0
                attn_mask_rel_pos = gate_a_1.view(bsz * self.total_num_qk_heads, -1, 1) * position_bias

            attn_mask_rel_pos = attn_mask_rel_pos.view((-1, seq_len, seq_len))
            attn_mask_rel_pos = attn_mask_rel_pos.reshape(bsz, self.total_num_qk_heads, seq_len, seq_len)[:, :, :, :]
    ...
    def prune(self):
        new_config = {
            "use_attention": True,
            "remaining_qk_heads": self.remaining_qk_heads,
            "remaining_vo_heads": self.remaining_vo_heads,
        }
        # if self.hard_concrete_for_layer is not None:
        #     assert not self.hard_concrete_for_layer.training
        #     layer_mask = self.hard_concrete_for_layer() # (1,)
        ...
        if self.gate_for_qk is not None:
            ...
            new_config["remaining_qk_heads"] = qk_mask.nonzero().squeeze(-1).tolist()
            new_config["remaining_vo_heads"] = vo_mask.nonzero().squeeze(-1).tolist()
            
            if len(new_config["remaining_vo_heads"]) == 0:
                new_config["use_attention"] = False
            else:
                full_index_qk = qk_mask.nonzero().squeeze(-1)  # 1D
                full_index_vo = vo_mask.nonzero().squeeze(-1)  # 1D
...
class FeedForward(Module):
    ...
    def __init__(
        ...
        if prune_intermediate:
            self.gate_for_intermediate = Gate(
        ...
        self.register_buffer('intermediate_mask', torch.ones(intermediate_features))
    ...
    def forward(self, x):
        ...
        Returns:
            x (Tensor): shape: `(batch, sequence_length, io_features)`
        """
        batch, seq_len, io_features = x.size()
        ...
        if self.gate_for_intermediate is not None:
            # Calculate channel scores from weight matrices
            w_in = self.intermediate_dense.weight
            w_out = self.output_dense.weight
            channel_score = (w_in**2).sum(dim=1) + (w_out**2).sum(dim=0)
        
            self.gate_for_intermediate.get_channel_score(channel_score = self.normalize(channel_score))

            # import pdb;pdb.set_trace() 
            self.intermediate_mask = self.gate_for_intermediate()
            
            
            # Assert mask contains binary values (0 or 1)
            mask_values = torch.unique(self.intermediate_mask)
            assert torch.all(mask_values == torch.tensor([0.0, 1.0], device=mask_values.device)) or \
                torch.all(mask_values == torch.tensor([0.0], device=mask_values.device)) or \
                torch.all(mask_values == torch.tensor([1.0], device=mask_values.device)), \
                "intermediate_mask must contain only binary values (0 or 1)"
            
            # Apply mask to intermediate output
            x = x * self.intermediate_mask.unsqueeze(0).unsqueeze(0)
            
            # Ensure intermediate and output have identical pruning patterns
            intermediate_zero = (self.intermediate_mask == 0)
            output_zero = (self.intermediate_mask == 0)
            assert torch.all(intermediate_zero == output_zero), "Pruned channels in intermediate and output must be identical"
    ...
    def prune_split_save(self):
        ...
        if self.gate_for_intermediate is not None:
            assert not self.gate_for_intermediate.training
            self.gate_for_intermediate.get_channel_score(channel_score = self.normalize(channel_score))
            
            interm_mask = self.gate_for_intermediate()
...
class Transformer(Module):
    ...
    #         if "remaining_qk_heads" in attention_config:
    #             pass
    #             # assert "remaining_vo_heads" in attention_config
    #             # new_config["remaining_qk_heads"].append(attention_config["num_qk"])
    #             # new_config["remaining_vo_heads"].append(attention_config["num_vo"])
    #         else:
    #             new_config["num_heads"].append(attention_config["num_heads"])
            
    #         # if "remaining_vo_heads" in attention_config:
    #         #     new_config["remaining_vo_heads"].append(attention_config["remaining_vo_heads"])
    #         # else:
    #         #     new_config["num_vo_heads"].append(attention_config["num_vo_heads"])
    ...
    def prune_split_save(self):
        new_config = defaultdict(list)
        for layer in self.layers:
            attention_config = layer.attention.prune_split_save()
            new_config["use_attention"].append(attention_config["use_attention"])
            if "remaining_heads" in attention_config:
                new_config["remaining_heads"].append(attention_config["remaining_heads"])
            else:
            ...
            ff_config = layer.feed_forward.prune_split_save()
            new_config["use_feed_forward"].append(ff_config["use_feed_forward"])
            new_config["ff_interm_features"].append(ff_config["ff_interm_features"])
            if not ff_config["use_feed_forward"]:
                layer.feed_forward = None
...
class Transformer_split_save(Module):
    def __init__(
        self,
        pos_conv_embed: Module,
        dropout: float,
...
class Encoder(Module):
    ...
    def prune_split_save(self, conv_out_index):
        """In-place pruning of submodules."""
        prune_layer_norm(self.feature_projection.layer_norm, conv_out_index)
        prune_linear_layer(self.feature_projection.projection, conv_out_index, "input")
        transformer_config = self.transformer.prune_split_save()
        return transformer_config

class Encoder_split_save(Module):
    def __init__(
        self,
        feature_projection: Module,
        transformer: Module,
...
def _get_encoder(
    ...
    layer_norm_first: bool,
    layer_drop: float,
    prune_attention_heads: bool = False, # whether to prune attention heads
    # prune_attention_qk_heads: bool = False,
    # prune_attention_vo_heads: bool = False,
...
def _get_encoder_split_save(
    ...
    for idx in range(num_layers):
        if use_attention[idx]:
            attention = SelfAttention_split_save(
                embed_dim=embed_dim,
                num_heads=num_heads[idx],
                head_dim=head_dim,
                dropout=attention_dropout,
    ...
    transformer = Transformer_split_save(
        pos_conv_embed=pos_conv,
        dropout=dropout,
        layers=encoder_layers,
        layer_norm_first=not layer_norm_first,
    ...
    return Encoder_split_save(feature_projection, transformer)
...
def _get_wavlm_encoder(
    ...
    remaining_heads_qk: List[List[int]],
    remaining_heads_vo: List[List[int]],
    num_buckets: int,
    max_distance: int,
    attention_dropout: float,
    ff_interm_features: List[int],
    ...
    for i in range(num_layers):
        if use_attention[i]:
            attention = WavLMSelfAttention(
                ...
                remaining_heads_qk=remaining_heads_qk[i],
                remaining_heads_vo=remaining_heads_vo[i],
                dropout=attention_dropout,
