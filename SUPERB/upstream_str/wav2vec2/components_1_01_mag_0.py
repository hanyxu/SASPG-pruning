...
class SelfAttention(Module):
    ...
    def normalize(self, sum_score: torch.Tensor) -> torch.Tensor:
        ...
        if not self.shrink_scale:
            ...
            normalized = (sum_score) / (max_val) + 1e-5
    ...
    def prune(self, prune_ratio):
        ...
        # 计算每个head的通道得分
        channel_score = (w_q**2).sum(dim=1) + (w_k**2).sum(dim=1) + (w_v**2).sum(dim=1) + (w_out**2).sum(dim=0)
        channel_score = channel_score.reshape(self.num_heads, -1, 1).sum(dim=1)  # 形状: (num_heads, 1)
        
        # if self.gate_for_heads is not None:
        # assert not self.gate_for_heads.training
        # 移除原gate_for_heads的得分传递逻辑（不再需要）
        # self.gate_for_heads.get_channel_score(channel_score = self.normalize(channel_score))
        
        # ========== 核心修改：基于channel_score生成head_mask ==========
        # 1. 归一化得分并压缩维度为 (num_heads,)
        norm_channel_score = self.normalize(channel_score).squeeze(-1)  # 去掉最后一维，形状: (num_heads,)
        # 2. 计算要保留的head数量（前50%），确保至少保留1个（避免全0）
        num_keep = max(1, int(self.num_heads * prune_ratio))
        # 3. 对得分降序排序，获取前num_keep个head的索引
        _, top_indices = torch.sort(norm_channel_score, descending=True)
        keep_indices = top_indices[:num_keep]
        # 4. 初始化全0的head_mask，将保留的索引设为1（保持和原始num_heads同尺寸）
        head_mask = torch.zeros_like(norm_channel_score, dtype=torch.float32)
...
class WavLMSelfAttention(SelfAttention):
    ...
    def prune(self, prune_ratio):
        new_config = {
            "use_attention": True,
            "remaining_heads": self.num_heads,
        }
        # if self.hard_concrete_for_layer is not None:
        #     assert not self.hard_concrete_for_layer.training
        #     layer_mask = self.hard_concrete_for_layer() # (1,)
        ...
        new_config["remaining_heads"] = head_mask.squeeze(-1).nonzero().squeeze(-1).tolist()
...
class FeedForward(Module):
    ...
    def normalize(self, sum_score: torch.Tensor) -> torch.Tensor:
        ...
        if not self.shrink_scale:
            ...
            # print('/project_bdda8/bdda/hnxu/prune/SSLprune/DPHuBERT_pretrain/run_100h_test_thre_lr_2e-4_320x2_gate_down_0.5_0.01_warm0.3_shrink_scale.sh')
            print('shrink scale')
            max_val = torch.max(sum_score)
    ...
    def prune(self, prune_ratio):
        new_config = {
            ...
            "ff_interm_features": 0
        }
        
        w_in = self.intermediate_dense.weight
        w_out = self.output_dense.weight
        channel_score = (w_in**2).sum(dim=1) + (w_out**2).sum(dim=0)  # 形状: (ff_interm_features,)
        
        # if self.gate_for_intermediate is not None:
        # assert not self.gate_for_intermediate.training
        # 移除原gate_for_intermediate的得分传递逻辑
        # self.gate_for_intermediate.get_channel_score(channel_score = self.normalize(channel_score))
        
        # ========== 核心修改：基于channel_score生成通道掩码 ==========
        # 1. 归一化得分
        norm_channel_score = self.normalize(channel_score)  # 形状: (ff_interm_features,)
        
        # 2. 计算要保留的通道数量（前 prune_ratio 比例），确保至少保留1个
        num_channels = self.intermediate_dense.out_features
        num_keep = max(1, int(num_channels * prune_ratio))
        
        # 3. 对得分降序排序，获取前num_keep个通道的索引
        _, top_indices = torch.sort(norm_channel_score, descending=True)
        keep_indices = top_indices[:num_keep]
        
        # 4. 初始化全0的通道掩码，将保留的索引设为1
        interm_mask = torch.zeros_like(norm_channel_score, dtype=torch.float32)
        interm_mask[keep_indices] = 1.0
        # print('interm_mask:', interm_mask)
        # ============================================================
        # import pdb;pdb.set_trace()
        interm_index = interm_mask.nonzero().squeeze(-1)  # 必须指定dim=-1
        # print('interm_mask.nonzero():', interm_mask.nonzero())
        # print('interm_mask.nonzero().squeeze(-1):', interm_mask.nonzero().squeeze(-1))
        # print('interm_index:', interm_index)
...
class Transformer(Module):
    ...
    def prune(self, prune_ratio):
        ...
        for layer in self.layers:
            attention_config = layer.attention.prune(prune_ratio=prune_ratio)
            ...
            ff_config = layer.feed_forward.prune(prune_ratio=prune_ratio)
...
class Encoder(Module):
    ...
    def prune(self, conv_out_index, prune_ratio):
        ...
        transformer_config = self.transformer.prune(prune_ratio=prune_ratio)
