...
class SelfAttention(Module):
    ...
    def forward(
        ...
        # if isinstance(self.num_heads, dict):
        #     proj_shape_qk = (batch_size * self.num_heads['num_qk'], -1)
        #     q = self.q_proj(x).view(batch_size,-1, self.num_heads['num_qk'])
        #     k = self.k_proj(x).view(batch_size,-1, self.num_heads['num_qk'])
        # else:
        proj_shape_qk = (batch_size * self.num_heads, -1, self.head_dim)
        ...
        if self.gate_for_qk is not None:
            # print('self.gate_for_qk is not None')
            ...
            # # print('self.qk_mask:', (self.qk_mask==0.).sum().item()/self.qk_mask.numel()*768)
        ...
        if self.final_distill:
            ...
            assert self.qk_mask is not None
            _, top_indices = torch.topk(channel_score_qk, k=self.qk_mask)  #
            # # print('self.qk_mask:',self.qk_mask)
            self.qk_mask = torch.zeros_like(channel_score_qk)  
            self.qk_mask[top_indices] = 1
        ...
        k = k * self.qk_mask
            ...
            # self.final_distill = False
        # if not isinstance(self.qk_mask, int):
        #     # print('self.qk_mask:',self.qk_mask.sum().item())
            
        #     q = q * self.qk_mask
        #     k = k * self.qk_mask
            
        #     # self.q_proj.weight = self.q_proj.weight * self.qk_mask
        #     # self.q_proj.bias = self.q_proj.bias  * self.qk_mask
            
        #     # self.k_proj.weight = self.k_proj.weight * self.qk_mask
        #     # self.k_proj.bias = self.k_proj.bias  * self.qk_mask
            
        # # print('mask q:', (q==0.).sum().item()/q.numel()*768)
        
        
        q = q.view(*proj_shape_qk)
        k = k.view(*proj_shape_qk)
        # if isinstance(self.num_heads, dict):
        #     proj_shape_vo = (batch_size * self.num_heads['num_vo'], -1)
        #     v = self.v_proj(x).view(batch_size,-1, self.num_heads['num_vo'])
        # else:
        proj_shape_vo = (batch_size * self.num_heads, -1, self.head_dim)
        proj_shape_vo_re = (batch_size , -1, self.head_dim * self.num_heads)
        ...
        weights = (self.scaling * q) @ k.transpose(2, 1)  # B, nH, L, L
        ...
        if attention_mask is not None:
            ...
            # if isinstance(self.num_heads, dict):
            #     import pdb;pdb.set_trace()
            #     weights = weights.view(batch_size, self.num_heads['num_qk'], length, length) + attention_mask
            #     weights = weights.view(batch_size*self.num_heads['num_qk'], length, length)
            # else:
            weights = weights.view(batch_size, self.num_heads, length, length) + attention_mask
        ...
        if self.gate_for_vo is not None:
            ...
            self.vo_mask = self.gate_for_vo()
            
            
            v = v * self.vo_mask
            
        elif self.final_distill:
        
            # import pdb;pdb.set_trace()
            # print('self.vo_mask:', self.vo_mask)
            w_v = self.v_proj.weight
            w_out = self.out_proj.weight
            
            channel_score_vo = (w_v**2).sum(dim=1) + (w_out**2).sum(dim=0)
            assert self.vo_mask is not None
            _, top_indices = torch.topk(channel_score_vo, k=self.vo_mask)  #
            self.vo_mask = torch.zeros_like(channel_score_vo)  
            self.vo_mask[top_indices] = 1

            self.final_distill = False

        v = v * self.vo_mask
        
        # if not isinstance(self.vo_mask, int):
        #     # print('self.vo_mask:',self.vo_mask.sum().item())
        #     # import pdb;pdb.set_trace()
        #     # self.v_proj.weight = self.v_proj.weight * self.vo_mask
        #     # self.v_proj.bias = self.v_proj.bias  * self.vo_mask
            
        #     # self.out_proj.weight = self.out_proj.weight * self.vo_mask.t()
            
        #     v = v * self.vo_mask
        last_dim_size = v.shape[-1]
        ...
        print("v index as :", nonzero_indices)

        # # print('mask v:', (v==0.).sum().item()/v.numel()*768)
        
        import pdb;pdb.set_trace()
        v = v.view(*proj_shape_vo)
        ...
        # if isinstance(self.num_heads, dict):
        #     output = output.view(batch_size, self.num_heads['num_vo'], length, self.head_dim)
        #     output = output.transpose(2, 1)
        #     output = output.reshape(batch_size, length, self.num_heads['num_vo'] * self.head_dim)
        # else:
        # import pdb;pdb.set_trace()
        # last_dim_size = output.shape[-1]
        ...
        #     if torch.any(matrix != 0):
        #         nonzero_indices.append(i)
        # output = output.view(batch_size, self.num_heads, length, self.head_dim)
        
        # output = output.transpose(2, 1)
        # output = output.reshape(batch_size, length, self.num_heads * self.head_dim)
        # import pdb;pdb.set_trace()
        ...
        print("output index as :", nonzero_indices)
            
        # # print('output before:', (output==0.).sum().item()/output.numel()*768)
        
        output = self.out_proj(output)

        # # print('output after:', (output==0.).sum().item()/output.numel()*768)
