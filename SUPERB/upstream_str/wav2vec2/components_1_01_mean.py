...
#     prune_layer_norm,
# )
from .gate_1_01_mean import Gate
from .pruning_utils import (
    prune_linear_layer,
    prune_conv1d_layer,
    prune_layer_norm,
...
class SelfAttention(Module):
    ...
    def forward(
        ...
        # print('attn_mean:', ((w_q**2).mean()+ (w_k**2).mean()+ (w_v**2).mean()+(w_out**2).mean())/4)
        channel_score = ((w_q**2).sum(dim=1) / w_q.shape[1]  + (w_k**2).sum(dim=1)  / w_k.shape[1] + (w_v**2).sum(dim=1) / w_v.shape[1] + (w_out**2).sum(dim=0) / w_out.shape[0]) / 4
        assert w_q.shape[1]==w_k.shape[1]==w_v.shape[1]==w_out.shape[0]==768
        channel_score_reshape = channel_score.reshape(self.num_heads, -1, 1)
        
        assert channel_score_reshape.shape[1] == 64
        channel_score = channel_score_reshape.sum(dim=1) / channel_score_reshape.shape[1]
        
        if self.gate_for_heads is not None:
            self.gate_for_heads.get_channel_score(channel_score = channel_score)
    ...
    def prune(self):
        ...
        assert w_q.shape[1]==w_k.shape[1]==w_v.shape[1]==w_out.shape[0]==768
        # channel_score = channel_score.reshape(self.num_heads, -1, 1).sum(dim=1) / self.num_heads
        channel_score_reshape = channel_score.reshape(self.num_heads, -1, 1)
        assert channel_score_reshape.shape[1] == 64
        channel_score = channel_score_reshape.sum(dim=1) / channel_score_reshape.shape[1]
...
class FeedForward(Module):
    ...
    def forward(self, x):
        ...
        # print('ffn_mean:', ((w_in**2).mean() + (w_out**2).mean()) / 2)
        channel_score = ((w_in**2).sum(dim=1) / w_in.shape[1] + (w_out**2).sum(dim=0) / w_out.shape[0] ) / 2
        # import pdb;pdb.set_trace()
        assert w_in.shape[1]==w_out.shape[0]==768
        
        if self.gate_for_intermediate is not None:
            self.gate_for_intermediate.get_channel_score(channel_score = (channel_score))
