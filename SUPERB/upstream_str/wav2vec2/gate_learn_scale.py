    ...
    def forward(self) -> torch.Tensor:
        ...
        if self.training:
            ...
            if self.eta_max == self.eta_min:
                ...
                if self.current_steps / self.total_steps <= 0.15:
                    self.tau = (self.eta_min + 0.5 * (self.eta_max - self.eta_min) * (1 + math.cos(math.pi * self.current_steps / int(self.total_steps*0.15))))
                else:
                    self.tau = self.eta_min + 0.5 * (self.eta_max - self.eta_min) * (1 - math.cos(math.pi * int(self.current_steps-0.15*self.total_steps) / int(self.total_steps*0.85)))
            ...
            if not self.reset_threshold:
                if not self.init_sparsity:
                    ...
                    new_value = (torch.sqrt(self.channel_score.min()) - 1e-6).clone().detach().to(device)
                    ...
                    print('learn scale threshold set:', new_value**2)
            ...
            if self.compiled_mask is None:
                ...
                if self.channel_score is None:
                    ...
                    # self.compiled_mask = (torch.round(torch.sigmoid((self.channel_score - self.gate_threshold**2))) ==1.0)
