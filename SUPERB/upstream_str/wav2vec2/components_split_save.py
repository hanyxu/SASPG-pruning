...
class SelfAttention_split_save(Module):
    ...
    def __init__(
        ...
        self.register_buffer('qk_mask', torch.ones(embed_dim))
        self.register_buffer('vo_mask', torch.ones(embed_dim))
    ...
    def forward(
        ...
        """
        if x.dtype == torch.float16 and self.q_proj.weight.dtype != torch.float16:
            self.q_proj = self.q_proj.half()
            self.k_proj = self.k_proj.half()
            self.v_proj = self.v_proj.half()
        ...
        if not hasattr(self, 'qk_mask'):
            raise ValueError("not fixed qk mask")
        
        # 获取保留的通道索引
        qk_keep_indices = torch.nonzero(self.qk_mask, as_tuple=True)[0]
        vo_keep_indices = torch.nonzero(self.vo_mask, as_tuple=True)[0]
        
        # torch.nonzero(self.qk_mask, as_tuple=True)[0]
        
        # 应用QKV投影（使用剪枝后的维度）
        q_pruned = self.q_proj(x)  # [batch_size, length, num_heads['num_qk']]
        k_pruned = self.k_proj(x)  # [batch_size, length, num_heads['num_qk']]
        v_pruned = self.v_proj(x)  # [batch_size, length, num_heads['num_vo']]
        
        # 创建完整维度的QKV张量，并用0填充未保留的通道

        q_full = torch.zeros(
        (batch_size, length, embed_dim),
        dtype=q_pruned.dtype,
        device=q_pruned.device
        )
        k_full = torch.zeros(
        (batch_size, length, embed_dim),
        dtype=k_pruned.dtype,
        device=k_pruned.device
        )
        v_full = torch.zeros(
        (batch_size, length, embed_dim),
        dtype=v_pruned.dtype,
        device=v_pruned.device
        )
        # import pdb;pdb.set_trace()
        # 将剪枝后的结果填充到完整张量中
        q_full[:, :, qk_keep_indices] = q_pruned
        k_full[:, :, qk_keep_indices] = k_pruned
        v_full[:, :, vo_keep_indices] = v_pruned
        
        # 重塑为多头注意力格式
        shape = (batch_size, length, 12, self.head_dim)
        q = q_full.view(*shape).transpose(2, 1)  # B, nH, L, Hd
        k = k_full.view(*shape).permute(0, 2, 3, 1)  # B, nH, Hd, L
        v = v_full.view(*shape).transpose(2, 1)  # B, nH, L, Hd
        ...
        # 应用vo_mask（如果需要）
        assert hasattr(self, 'vo_mask')
        if hasattr(self, 'vo_mask'):
            output = output * self.vo_mask.unsqueeze(0).unsqueeze(0)
        
        # 应用输出投影（使用剪枝后的维度）
        output_pruned = output[:, :, vo_keep_indices]  # [batch_size, length, num_heads['num_vo']]
        output_final = self.out_proj(output_pruned)
    ...
    def get_num_params(self):
        ...
        # 计算qk投影的参数量
        if hasattr(self, 'qk_mask'):  # final_distill模式下使用固定掩码
            num_qk = int(self.qk_mask.sum().item())
        ...
        # 计算vo投影的参数量
        if hasattr(self, 'vo_mask'):  # final_distill模式下使用固定掩码
            num_vo = int(self.vo_mask.sum().item())
    ...
    def prune_split_save(self):
        ...
        # 确保已设置final_distill模式
        if not self.final_distill or not hasattr(self, 'qk_mask'):
        ...
        # 更新embed_dim为修剪后的维度
        self.original_qk_dim = self.embed_dim
    ...
    def prune(self):
        ...
        # 确保已设置final_distill模式
        if not hasattr(self, 'qk_mask'):
            raise ValueError("use set_final_distill() first")
        ...
        print(f"[Model Pruned] channel from {self.original_qk_dim} to {self.embed_dim}")
        print(f"save channel: qk={len(qk_keep_indices)}, vo={len(vo_keep_indices)}")
    ...
    def forward(
        ...
        if self.final_distill:
            ...
            if self.training and not hasattr(self, '_qk_mask_initialized'):
                ...
                self.register_buffer('qk_mask', qk_mask)
                ...
                self.register_buffer('vo_mask', vo_mask)
            ...
            # 使用固定掩码
            if hasattr(self, 'qk_mask'):
                qk_mask = self.qk_mask
            if hasattr(self, 'vo_mask'):
                vo_mask = self.vo_mask
        else:
...
class FeedForward(Module):
    ...
    def forward(self, x):
        ...
        if self.gate_for_intermediate is not None:
            ...
            intermediate_mask = self.gate_for_intermediate()   # (intermediate_features,)
            # tensor([0.4999, 0.5049, 0.5041,  ..., 0.5027, 0.5035, 0.4988], device='cuda:0') 
