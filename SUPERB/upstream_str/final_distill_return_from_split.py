...
from wav2vec2.model_1_01_return_from_split import (
    wav2vec2_model_split_save, wav2vec2_model, 
)

_LG = logging.getLogger(f"{__name__}:{_get_rank()}")

# def set_attn_masks(model, attn_mask=None):
...
#         if hasattr(module, 'qk_unmask_num'):
#             print('num_vo_list[i]:',num_vo_list[layer_index])
...
def set_final_distill(model, final_distill=True):
    def apply_final_distill(module):
        if hasattr(module, 'final_distill'):
            module.final_distill = final_distill
    
    for name, module in model.named_modules():
        apply_final_distill(module)
...
def run_train(args):
    ...
    print('student_ckpt["config"]:', student_ckpt["config"])
    student_model = wav2vec2_model_split_save(**student_ckpt["config"])
    # attn_mask = student_ckpt["attn_mask"]
    student_model = set_norm_type(student_model, norm_type=args.norm_type)
    student_model = set_final_distill(student_model, final_distill=True)
    _LG.info(f"Student model:\n{student_model}")
    # student_model = set_attn_masks(student_model, attn_mask=attn_mask)
    student_result = student_model.load_state_dict(student_ckpt["state_dict"], strict=True)
