...
root_dir=exp_minmax_large/hubert-large_${train_subset}_sp${target_sparsity}_spup${sparsity_warmup}_lr${lr}_up${warmup}_max${max}_${distill_mode}${distill_layers}_reglr${reg_lr}_gatelr${gate_lr}_${pruning_units}_sec${seconds_per_batch}_accum${accum_grad}_gate_norm_${eta_max}_${eta_min}_tau_gate_up_30%_down
...
#     mem=$(nvidia-smi -i 1 --query-gpu=memory.free --format=csv,noheader,nounits | xargs)
